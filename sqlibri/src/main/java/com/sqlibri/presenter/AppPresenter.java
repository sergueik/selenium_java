...
public class AppPresenter {
	...
	public void init(Stage primaryStage) {
		...
		commands.valueProperty().addListener(
				e -> editor.pasteCode(commands.getSelectionModel().getSelectedItem()));
	}
	...
	public void loadQuery() {
		...
		try {
			query = Files.readAllLines(file.toPath()).stream()
	...
	public void exportCSV() {
		...
		try {
			Files.write(file.toPath(), CSVParser
					.parseToCSV(lastResult.getColumnNames(), lastResult.getTableData())
					.getBytes());
		} catch (IOException e) {
			showErrorDialog("FILE IO ERROR:", e.getMessage());
		} catch (Exception e) {
	...
	public void showUserGuide() {
		...
		try {
			accordion = FXMLLoader.load(this.getClass().getClassLoader()
	...
	private void loadTables(File database) {
		Image rootImg = new Image(
				getClass().getClassLoader().getResourceAsStream("image/database.png"),
				16, 16, false, false);
		TreeItem<String> rootItem = new TreeItem<>(database.getName(),
				new ImageView(rootImg));
		rootItem.setExpanded(true);
		Image tableImg = new Image(
				getClass().getClassLoader().getResourceAsStream("image/table.png"), 16,
				16, false, false);
		List<String> tables = new Database(database).getAllTables();

		List<TreeItem<String>> tablesList = tables.stream()
	...
	private void loadTableView(QueryResult queryResult) {
		...
		for (int column = 0; column < queryResult.getColumnCount(); column++) {
			...
			TableColumn<ObservableList<String>, String> col = new TableColumn<>(
					queryResult.getColumnNames().get(j));
