...
class Utils {
	...
	/**
	 * Injected by
	 * https://wiki.jenkins-ci.org/display/JENKINS/GitHub+pull+request+
	 * builder+plugin
	 */
	public static final String GIT_PR_ID_ENV_PROPERTY = "ghprbPullId";

	public static final Pattern HTTP_GITHUB_USER_REPO_PATTERN = Pattern
			.compile("^(http[s]?://[^/]*)/([^/]*/[^/]*).*");
	public static final Pattern SSH_GITHUB_USER_REPO_PATTERN = Pattern
			.compile("^.+:(.+)");
	...
	public static String getUserRepo(final String url) {
		...
		if (userRepo == null) {
			throw new IllegalStateException(String.format(
					"Invalid GitHub project url: %s", url));
		}
	...
	public static String getGitUrl(Run build, TaskListener listener)
			throws IOException, InterruptedException {
		final EnvVars envVars = build.getEnvironment(listener);
		return envVars.get(GIT_URL_ENV_PROPERTY);
	}

	public static String getBuildUrl(Run build, TaskListener listener)
			throws IOException, InterruptedException {
		final EnvVars envVars = build.getEnvironment(listener);
		return envVars.get(BUILD_URL_ENV_PROPERTY);
	}

	public static Integer gitPrId(Run build, TaskListener listener)
			throws IOException, InterruptedException {
		final EnvVars envVars = build.getEnvironment(listener);
