...
class CoberturaParser implements CoverageReportParser {
	...
	private static String findFirst(String string, String pattern) {
		...
		if (result != null) {
			...
			throw new IllegalArgumentException("Can't find " + pattern + " in "
					+ string);
		}
	}
	...
	public float get(String coberturaFilePath) {
		try {
			...
			float lineRate = Float.parseFloat(findFirst(content,
					"line-rate=\"([0-9.]+)\""));
			float branchRate = Float.parseFloat(findFirst(content,
					"branch-rate=\"([0-9.]+)\""));
			float linesValid = parseFloatOrDefault(
					findFirstOrNull(content, "lines-valid=\"([0-9.]+)\""), 0);
			float branchesValid = parseFloatOrDefault(
					findFirstOrNull(content, "branches-valid=\"([0-9.]+)\""), 0);
