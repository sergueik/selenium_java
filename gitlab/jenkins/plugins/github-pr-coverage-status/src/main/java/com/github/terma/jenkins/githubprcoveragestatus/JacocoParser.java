...
class JacocoParser implements CoverageReportParser {
	...
	private float getByXpath(final String filePath, final String content,
			final String xpath) {
		try {
	...
	public float get(final String jacocoFilePath) {
		...
		try {
			...
			throw new IllegalArgumentException("Can't read Jacoco report by path: "
					+ jacocoFilePath);
		}

		final float lineMissed = getByXpath(jacocoFilePath, content, MISSED_XPATH);
		final float lineCovered = getByXpath(jacocoFilePath, content,
				COVERAGE_XPATH);
