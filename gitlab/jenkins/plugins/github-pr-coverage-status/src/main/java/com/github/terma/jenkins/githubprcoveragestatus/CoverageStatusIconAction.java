...
public class CoverageStatusIconAction implements UnprotectedRootAction {
	...
	/**
	 * Used by Jenkins Stapler service when get request on URL
	 * jenkins_host/getUrlName()
	 * 
	 * @param request
	 *          - request
	 * @param response
	...
	public void doIndex(StaplerRequest request, StaplerResponse response)
		...
		final float masterCoverage = Float.parseFloat(request
				.getParameter("masterCoverage"));
		...
		String color;
		if (coveragePercent < Configuration.getYellowThreshold())
			color = "#b94947"; // red
		else if (coveragePercent < Configuration.getGreenThreshold())
			color = "#F89406"; // yellow
		else
			color = "#97CA00"; // green
