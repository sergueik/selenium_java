...
class CachedGitHubRepository {
	...
	private static final transient Logger logger = Logger
			.getLogger(CachedGitHubRepository.class.getName());

	private transient GHRepository gitHubRepository;

	public GHPullRequest getPullRequest(final String gitHubUrl, final int prId)
			throws IOException {
		if (gitHubRepository == null)
			initGitHubRepository(gitHubUrl);
		return gitHubRepository.getPullRequest(prId);
	}

	private GitHub getGitHub() throws IOException {
		final SettingsRepository settingsRepository = ServiceRegistry
				.getSettingsRepository();
		final String apiUrl = settingsRepository.getGitHubApiUrl();
		final String personalAccessToken = settingsRepository
