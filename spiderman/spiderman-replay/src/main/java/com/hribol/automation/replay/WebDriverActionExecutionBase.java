 ...
 */
public abstract class WebDriverActionExecutionBase
		implements WebDriverActionExecution {

	public WebDriverActionExecutionBase(
	...
	public void execute(TestScenario testScenario)
		...
		try {
			...
			while (testScenario.hasMoreSteps()) {
				if (ConfigurationUtils
						.toSeconds(System.nanoTime() - elapsedTime) > timeout) {
					this.automationResult = AutomationResult.TIMEOUT;
					throw new TimeoutException("Could not execute the action! Waited "
							+ String.valueOf(System.nanoTime() - elapsedTime) + " to do "
							+ testScenario.nextActionName() + " http queries in queue: "
							+ httpRequestQueue.size());
				}
		...
		this.loadingTimes = new LoadingTimes(waitingTimes,
				testScenario.getActions());
	}
	...
	@Override
	public void dumpLoadingTimes(String fileNameToDump)
	...
	@Override
	public AutomationResult executeOnScreen(TestScenario testScenario, int i,
			VirtualScreenProcessCreator virtualScreenProcessCreator,
			String loadingTimesFileName, String harTimesFileName) {
		Process process;
	...
	private WebDriverActionExecutionBase(String baseURI,
			String pathToDriverExecutable, int timeout,
		...
		replayRequestFilter = new ReplayRequestFilter(this::setLock, baseURI,
				httpRequestQueue);
	...
	private void executeIgnoringExceptions(WebDriverAction webDriverAction)
		...
		while (i < maxRetries && (ConfigurationUtils
				.toSeconds(System.nanoTime() - startTime) < timeout)) {
			try {
		...
		throw new TimeoutException(
				"Could not execute the action! " + webDriverAction.getName());
	}
