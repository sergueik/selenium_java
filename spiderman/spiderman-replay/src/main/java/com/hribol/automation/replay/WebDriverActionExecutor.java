...
public class WebDriverActionExecutor {
	...
	public String getPathToDriverExecutable() throws IOException {
		pathToDriverExecutable = Optional.ofNullable(pathToDriverExecutable)
				.orElse(System.getenv(DRIVER_EXECUTABLE));
		if (pathToDriverExecutable == null) {
			throw new IOException(
					"Path to driver executable not set. Please either set it using"
	...
	public WebDriverActionExecutor pathToDriverExecutable(
	...
	public WebDriverActionExecutor measurementsPrecisionInMilliseconds(
	...
	public String getBaseURI() throws IOException {
		if (baseURI == null) {
			throw new IOException(
					"Base URI is not set. Please use the baseURI method");
		}
