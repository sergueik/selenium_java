...
public class DbAuditPublisherDescriptorImpl extends
	...
	private final static Logger LOGGER = Logger
			.getLogger(DbAuditPublisherDescriptorImpl.class.getName());
	...
	public DbAuditPublisherDescriptorImpl(
			final Class<? extends DbAuditPublisherImpl> clazz) {
		super(clazz);
		load();
	}
	...
	public boolean configure(final StaplerRequest req, final JSONObject json)
		...
		if (this.useJndi) {
			...
			this.jndiPassword = Scrambler
					.scramble(datasourceDetails.getString("jndiPassword"));
		} else {
			...
			this.jdbcPassword = Scrambler
					.scramble(datasourceDetails.getString("jdbcPassword"));
		}
		save();
	...
	public FormValidation doTestJdbcConnection(
		...
		LOGGER.log(Level.FINE,
				String.format("doTestJdbcConnection('%s','%s','%s','*****'", jdbcDriver,
						jdbcUrl, username));
		FormValidation retval = FormValidation
				.ok(Messages.DbAuditPublisherDescriptor_ConnectionOk());

		try {
			final Properties props = HibernateUtil.getExtraProperties(jdbcDriver,
					jdbcUrl, username, password);

			final Session session = HibernateUtil.getSessionFactory(props)
					.getCurrentSession();
	...
	public FormValidation doGenerateDdl(
			...
			@QueryParameter("audit2db.jdbcPassword") final String password)
			throws IOException, ServletException {
		LOGGER.log(Level.FINE, String.format("doGenerateDdl('%s','%s','%s','*****'",
				jdbcDriver, jdbcUrl, username));
		FormValidation retval;
		try {
			final String ddlText = HibernateUtil.getSchemaDdl(jdbcDriver, jdbcUrl,
					username, password);
