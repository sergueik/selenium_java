...
public class Audit2DbGraphListener implements GraphListener {
	...
	private static final Logger LOGGER = Logger
			.getLogger(Audit2DbGraphListener.class.getName());

	private BuildDetailsRepository repository;
	private BuildDetails details;

	public Audit2DbGraphListener(BuildDetailsRepository repository,
	...
	public void onNewHead(FlowNode node) {
		...
		if (!CpsFlowExecution.class.isInstance(execution)) {
			throw new IllegalStateException(
					"Cannot get a pipeline result from a FlowExecution that is not an instance of CpsFlowExecution: "
		...
		BuildDetailsResolver.updateResultAndEndTime(details, cpsFlowExecution,
				new Date());
