 ...
 *
 */
public class BuildDetailsHibernateRepository extends AbstractHibernateRepository
	...
	public void saveBuildDetailsList(final List<BuildDetails> details) {
		...
		final TransactionStatus tx = getTransactionManager().getTransaction(null);
		try {
	...
	public List<BuildDetails> getBuildDetailsByDateRange(final Date start,
		...
		criteria.add(Restrictions.or(
				Restrictions.and(Restrictions.ge("startDate", inclusiveStartDate),
						Restrictions.le("startDate", inclusiveEndDate)),
				Restrictions.and(Restrictions.ge("endDate", inclusiveStartDate),
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<BuildDetails> getBuildDetailsByParams(final String masterHostName,
			final String paramName, final String paramValue, final Date fromDate,
			final Date toDate) {
		...
		final DetachedCriteria criteria = DetachedCriteria
				.forClass(BuildDetails.class).createAlias("node", "node")
		...
		if ((null == paramName) || paramName.isEmpty()) {
			...
			criteria.createAlias("parameters", "param")
					.add(Restrictions.and(Restrictions.ilike("param.name", paramName),
