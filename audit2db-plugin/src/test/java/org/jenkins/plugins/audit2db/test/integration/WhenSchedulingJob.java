...
public class WhenSchedulingJob extends HudsonTestCase {
	...
	public void testPlainJobShouldSaveNoParameters() throws Exception {
		...
		Assert.assertEquals("Unexpected build result", Result.SUCCESS,
				build.getResult());
		...
		Assert.assertEquals("Unexpected number of params", 0,
	...
	public void testParameterisedJobShouldSaveAllParameters() throws Exception {
		...
		// set parameters
		final ParameterDefinition param1 = new StringParameterDefinition(
				"myStringParam", "myStringValue", "My String Parameter");
		final ParameterDefinition param2 = new BooleanParameterDefinition(
				"myBooleanParam", false, "My Boolean Parameter");
		...
		Assert.assertNotNull("Unexpected null end date", actual.getEndDate());
		Assert.assertTrue("Unexpected duration", actual.getDuration() > 0L);
		Assert.assertEquals("Unexpected number of params", 2,
				actual.getParameters().size());
	}
}
