...
public class TestUtils {
	...
	private static final BuildDetailsRepository repository = new BuildDetailsHibernateRepository(
			HibernateUtil.getSessionFactory(HibernateUtil
					.getExtraProperties(JDBC_DRIVER, JDBC_URL, JDBC_USER, JDBC_PASS)));
	...
	public static BuildDetails createRandomBuildDetails(
		...
		final BuildNode node = new BuildNodeImpl("NODE ADDRESS", "NODE HOSTNAME",
				"NODE DISPLAYNAME", "NODE URL", "NODE NAME", "NODE DESCRIPTION",
				"NODE LABEL");
		build.setNode(node);
	...
	public static List<BuildDetails> createRandomBuildHistory(
			final String hostName, final String projectName, final int numOfBuilds,
			final boolean withParams) {
	...
	public static Map<String, List<BuildDetails>> createRandomDataset(
		...
		for (int projCtr = 1; projCtr <= numOfProjects; projCtr++) {
			...
			final List<BuildDetails> details = createRandomBuildHistory(hostName,
					projectName, numOfBuilds, withParams);
	...
	public static <T extends DbAuditReport> T getReportExtension(
			final Class<T> reportClass) {
		final ExtensionList<DbAuditReport> reportExtensions = Jenkins.getInstance()
		...
		throw new RuntimeException(
				"Unable to find extension of type " + reportClass);
	}
}