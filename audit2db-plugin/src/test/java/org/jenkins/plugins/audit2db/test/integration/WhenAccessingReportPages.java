...
public abstract class WhenAccessingReportPages extends HudsonTestCase {
	private final static String AUDITOR_NAME = String
			.format("AUDITOR_" + (new Date()).getTime());
	...
	private AuthorizationStrategy getAuthStrategy() {
		if (null == authStrategy) {
			authStrategy = new AuditReportsAuthorizationStrategy(
					jenkins.getAuthorizationStrategy(), AUDITOR_NAME, DbAuditPlugin.RUN);
		}
		return authStrategy;
	}
	...
	protected void shouldGenerateErrorForAnonymousUser(
		...
		try {
			page.load();
			fail(
					"Unexpected successful report access. The page should be inaccessible without the valid permissions.");
		} catch (final Exception e) {
			// expecting HTTP status 403 - forbidden
			assertEquals(
					String.format("Unexpected exception type: %s !", e.getMessage()),
	...
	protected void shouldGenerateErrorForNonAuditorUser(
		...
		try {
			...
			page.load();
			fail(
					"Unexpected successful report access. The page load should have failed without the valid permissions.");
		} catch (final Exception e) {
			// expecting HTTP status 403 - forbidden
			assertEquals(
					String.format("Unexpected exception type: %s !", e.getMessage()),
