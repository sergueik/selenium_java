...
public class WhenTestingDataSource extends HudsonTestCase {
	...
	public void testValidJdbcDatasourceShouldSucceed() throws Exception {
		...
		final DbAuditPublisherDescriptor descriptor = (DbAuditPublisherDescriptor) publisher
				.getDescriptor();
		final FormValidation testResult = descriptor
				.doTestJdbcConnection(jdbcDriver, jdbcUrl, jdbcUser, jdbcPassword);
		Assert.assertEquals("Unexpected connection error.", FormValidation.Kind.OK,
				testResult.kind);
	}
	...
	public void testInvalidJdbcDriverShouldFail() throws Exception {
		...
		// final DbAuditPublisher publisher = new DbAuditPublisherImpl();
		// final DbAuditPublisherDescriptor descriptor =
		// (DbAuditPublisherDescriptor) publisher.getDescriptor();
		// final FormValidation testResult = descriptor.doTestJdbcConnection(
		// "WrongDriver", jdbcUrl, jdbcUser, jdbcPassword);
		// Assert.assertEquals("Unexpected successful connection.",
		// FormValidation.Kind.ERROR, testResult.kind);
	}

	@Test
	public void testGeneratingDdlWithValidJdbcDetailsShouldSucceed()
			throws Exception {
		final DbAuditPublisher publisher = new DbAuditPublisherImpl();
		final DbAuditPublisherDescriptor descriptor = (DbAuditPublisherDescriptor) publisher
				.getDescriptor();

		final FormValidation testResult = descriptor.doGenerateDdl(jdbcDriver,
	...
	// {
	// final SimpleNamingContextBuilder contextBuilder =
	...
	//
	// @Test
	// public void testingValidJndiDatasourceWithWrongLoginShouldFail() throws
	// Exception {
	// final SimpleNamingContextBuilder contextBuilder =
	// SimpleNamingContextBuilder.emptyActivatedContextBuilder();
	// contextBuilder.bind(jndiName, getDatasource());
