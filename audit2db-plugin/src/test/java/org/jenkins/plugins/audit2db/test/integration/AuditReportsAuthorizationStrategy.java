...
public class AuditReportsAuthorizationStrategy extends AuthorizationStrategy {
	...
	public AuditReportsAuthorizationStrategy(final AuthorizationStrategy template,
	...
	public ACL getRootACL() {
		return new ACL() {
			...
			public boolean hasPermission(final Authentication auth,
				...
				if (retval) {
					if (0 == Permission.ID_COMPARATOR.compare(requestedPermission,
							AuditReportsAuthorizationStrategy.this.permission)) {
						retval = retval && requestedUser
								.equalsIgnoreCase(AuditReportsAuthorizationStrategy.this.user);
					}
				}
