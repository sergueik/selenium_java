...
public class BuildParameterImplTests {
	private final BuildDetails details = new BuildDetailsImpl("BUILDID",
			"BUILD NAME", "BUILD_FULLNAME", new Date(), new Date(), 10L, "USERID",
			"USERNAME", null, null);

	private final BuildDetails otherDetails = new BuildDetailsImpl("BUILDIDXXX",
			"BUILD NAME", "BUILD_FULLNAME", new Date(), new Date(), 10L, "USERID",
			"USERNAME", null, null);

	private final BuildParameter expected = new BuildParameterImpl("PARAM_ID",
			"PARAM NAME", "PARAM VALUE", details);

	@Test
	public void differentAttributesShouldPreserveEquality() {
		final BuildParameter actual = new BuildParameterImpl(expected.getId(),
				expected.getName() + "DIFFERENT", expected.getValue() + "DIFFERENT",
				otherDetails);
	...
	public void differentIdShouldBreakEquality() {
		final BuildParameter actual = new BuildParameterImpl(
				expected.getId() + "DIFFERENT", expected.getName(), expected.getValue(),
