...
public class JobsByParamReportTests {
	...
	public void matchingParametersShouldReturnNonEmptyResults() {
		...
		final BuildParameter param = dataset.values().iterator().next().get(0)
				.getParameters().get(0);

		final Map<String, List<BuildDetails>> results = report.getProjectExecutions(
				param.getName(), param.getValue(), TestUtils.NOW, TestUtils.NOW);
	...
	public void paramNameWithWildcardShouldReturnNonEmptyResults() {
		...
		final BuildParameter param = dataset.values().iterator().next().get(0)
				.getParameters().get(0);
		final Map<String, List<BuildDetails>> results = report.getProjectExecutions(
				param.getName().substring(0) + "%", param.getValue(), TestUtils.NOW,
				TestUtils.NOW);
		Assert.assertFalse("Unexpected empty results", results.isEmpty());
	}
	...
	public void paramValueWithWildcardShouldReturnNonEmptyResults() {
		...
		final BuildParameter param = dataset.values().iterator().next().get(0)
				.getParameters().get(0);
		final Map<String, List<BuildDetails>> results = report.getProjectExecutions(
				param.getName(), param.getValue().substring(0) + "%", TestUtils.NOW,
				TestUtils.NOW);
		Assert.assertFalse("Unexpected empty results", results.isEmpty());
	}
	...
	public void invalidParamNameShouldReturnEmptyResults() {
		...
		final Map<String, List<BuildDetails>> results = report.getProjectExecutions(
				param.getName() + " INVALID", param.getValue(), TestUtils.NOW,
				TestUtils.NOW);
	...
	public void invalidParamValueShouldReturnEmptyResults() {
		...
		final Map<String, List<BuildDetails>> results = report.getProjectExecutions(
				param.getName(), param.getValue() + " INVALID", TestUtils.NOW,
				TestUtils.NOW);
