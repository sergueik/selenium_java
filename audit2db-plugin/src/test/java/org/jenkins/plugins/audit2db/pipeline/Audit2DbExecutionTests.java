...
public class Audit2DbExecutionTests {
	...
	@Test
	public void testRunAddsListenerWithValidHibernateRepositoryToStepContext()
			throws Exception {
		Audit2DbStep step = new Audit2DbStep(JDBC_DRIVER, JDBC_URL, JDBC_USER,
				JDBC_PASS);
		...
		assertThat("Null BuildDetailsRepository", listener, hasProperty(
				"repository", Matchers.isA(BuildDetailsHibernateRepository.class)));
	}

	@Test
	public void testRunAddsListenerWithValidBuildDetailsToStepContext()
			throws Exception {
		Audit2DbStep step = new Audit2DbStep(JDBC_DRIVER, JDBC_URL, JDBC_USER,
				JDBC_PASS);
		final List<Audit2DbGraphListener> listeners = new ArrayList<Audit2DbGraphListener>();
		...
		verify(flowExecution).addListener(listener);
		assertThat("No BuildDetails", listener,
				hasProperty("details", Matchers.isA(BuildDetails.class)));
		BuildDetails details = listener.getDetails();
		assertNotNull("Null BuildDetails", details);
		assertEquals("Mis-matched name", "Full Display Name # -99",
				details.getFullName());
		assertNotNull("Null BuildDetails.node", details.getNode());
		assertEquals("Mis-matched node url",
				"http://unconfigured-jenkins-server/computer/(master)/",
				details.getNode().getUrl());
	}
