...
public class WhenConfiguringPlugin extends HudsonTestCase {
	...
	// page.load();
	//
	// Assert.assertTrue("The useJndi flag was not set to true.",
	// page.isUseJndi());
	// Assert.assertEquals("Mismatched datasource name", datasourceName,
	// page.getJndiDatasource());
	...
	public void testSecurityMatrixShouldIncludeAuditReportsPermissions() {
		// Test is executed and fails in the presence of the @Ignore attribute
		// Assert.assertTrue("Audit reports permission options seem to be
		// missing",page.getAuditReportsPermissionColumnNumber() >= 0);
	}
}
