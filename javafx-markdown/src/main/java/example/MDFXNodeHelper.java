...
public class MDFXNodeHelper extends VBox {
	...
	class MDParser {
		...
		NodeVisitor visitor = new NodeVisitor(
				...
				new VisitHandler<>(com.vladsch.flexmark.ast.TextBase.class,
						this::visit),
		...
		public void visit(Heading heading) {
			...
			if (heading.getLevel() == 1 || heading.getLevel() == 2) {
				...
				for (int i = heading.getLevel() + 1; i <= currentChapter.length
						- 1; i += 1) {
					currentChapter[i] = 0;
				}
				;
		...
		public void visitListItem(String text,
		...
		public void visit(Paragraph paragraph) {
			...
			List<AttributesNode> atts = AttributesExtension.NODE_ATTRIBUTES
					.getFrom(document).get(paragraph);
			newParagraph();
			flow.getStyleClass().add("markdown-normal-flow");
			setAttrs(atts, true);
		...
		public void setAttrs(List<AttributesNode> atts, boolean add) {
			...
			atts3.forEach(att -> {
				if (att.getName().toLowerCase().equals("style")) {
					if (add)
						styles.add(att.getValue().toString());
					else
						styles.remove(att.getValue().toString());
				}
				if (att.isClass()) {
					if (add)
						elemStyleClass.add(att.getValue().toString());
					else
						elemStyleClass.remove(att.getValue().toString());
				}
			});
		}
