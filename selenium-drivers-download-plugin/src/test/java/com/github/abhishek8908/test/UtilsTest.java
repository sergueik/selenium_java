...
public class UtilsTest {
	...
	private static final String data = ""
      ...
      + "<head>"
			...
			+ "text-resize:100%;"+ "}"+ "</style>"
			+ "</head>"
			+ "<body style=\"margin:0; padding:0;\"bgcolor=\"#ededeb\"link=\"#0086da\">"
			+ "<style type=\"text/css\">"+ "* {"
			+ "-webkit-text-size-adjust:none;"
			+ "-webkit-text-resize:100%;"+ "text-resize:100%;"
			+ "}"+ "</style>"
	...
	public void testJsoupNavigation() {
		...
		passwordNodeSiblings = doc.select(
				String.format("body table span > strong:contains(%s):eq(0)", "Password:"));
