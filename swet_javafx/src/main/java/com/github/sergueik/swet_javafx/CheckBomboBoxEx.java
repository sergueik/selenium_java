...
// origin: https://github.com/controlsfx/controlsfx/blob/master/controlsfx-samples/src/main/java/org/controlsfx/samples/checked/HelloCheckComboBox.java
...
public class CheckBomboBoxEx extends ControlsFXSample {
	...
	public String getSampleDescription() {
		return "A simple UI control that makes it possible to select zero or "
				...
				+ "row - simply use the check model property to request the " + "current selection state.";
	}
	...
	public Node getPanel(Stage stage) {
		...
		checkComboBox.getCheckModel().getCheckedItems().addListener((ListChangeListener<String>) change -> {
			...
			while (change.next()) {
				...
				System.out.println("Added " + change.wasAdded() + " Permutated " + change.wasPermutated() + " Removed "
						+ change.wasRemoved() + " Replaced " + change.wasReplaced() + " Updated "
						+ change.wasUpdated());
				System.out.println("============================================");
			}
		});
