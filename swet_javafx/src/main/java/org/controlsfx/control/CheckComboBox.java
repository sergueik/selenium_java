...
/**
 * A simple UI control that makes it possible to select zero or more items
 * within a ComboBox-like control. Each row item shows a {@link CheckBox}, and
 * the state of each row can be queried via the {@link #checkModelProperty()
 * check model}.
 * 
 * <br>
 * The title shown in the combobox is, by default, a concatenation of the
 * selected items but this behaviour can be changed and it is possible to set a
 * fixed title (see {@link #titleProperty() } property), with or without an
 * indication of how many items have been checked (see
 * {@link #showCheckedCountProperty() } property).
 * 
 * <h3>Screenshots</h3>
 * <p>
 ...
 * 
 * <br>
 * <img src="checkComboBox.png" alt="Screenshot of CheckComboBox"> <br>
 * <p>
 ...
 * 
 * <br>
 * <img src="checkComboBoxWithCheckedItemCount.png" alt="Screenshot of
 * CheckComboBox with number of checked items">
 * 
 * <h3>Code Example:</h3>
 * <p>
...
public class CheckComboBox<T> extends ControlsFXControl {
	...
	/**
	 * Returns the {@link BooleanProperty} for a given item in the CheckComboBox.
	 * This is useful if you want to bind to the property.
	 */
	...
	// --- Check Model
	private ObjectProperty<IndexedCheckModel<T>> checkModel = new SimpleObjectProperty<>(this, "checkModel"); //$NON-NLS-1$

	/**
	 * Sets the 'check model' to be used in the CheckComboBox - this is the code
	 * that is responsible for representing the selected state of each
	 * {@link CheckBox} - that is, whether each {@link CheckBox} is checked or not
	 * (and not to be confused with the selection model concept, which is used in
	 * the ComboBox control to represent the selection state of each row)..
	 */
	...
	/**
	 * The check model provides the API through which it is possible to check single
	 * or multiple items within a CheckComboBox, as well as inspect which items have
	 * been checked by the user. Note that it has a generic type that must match the
	 * type of the CheckComboBox itself.
	 */
	...
	/**
	 * A {@link StringConverter} that, given an object of type T, will return a
	...
	/**
	 * The title to use for this control. If a non null value is explicitly set by
	 * the client, then that string will be used, otherwise a title will be
	 * constructed concatenating the selected items
	 */
	...
	/**
	 * Sets the title to use. If it is not null it will be used as title, otherwise
	 * title will be constructed by the skin
	 * 
	...
	/**
	 * A boolean to decide if the information of how many items are checked should
	 * be shown beside the fixed title. If a {@link #titleProperty()} has been set
	 * and this property is set to true then a string like (3/10) would be shown
	 * when 3 items out of 10 are checked.<br>
	 * This property has effect only if a fixed title has been set (see
	 * {@link #titleProperty()}), otherwise the title is constructed with a
	 * concatenation of the selected items.
	 * 
	...
	/**
	 * @return whether the checked items count is set to be shown beside a fixed
	 *         title
	 */
