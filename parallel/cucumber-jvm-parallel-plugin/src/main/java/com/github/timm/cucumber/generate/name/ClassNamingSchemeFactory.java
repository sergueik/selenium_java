...
public class ClassNamingSchemeFactory {
	...
	/**
	 * @param counter
	 *          Counter for adding 1-up numbers to generated class names.
	 */
	 ...
	 * @param namingPattern
	 *          Only required if <code>pattern</code> naming scheme is used. The
	 *          pattern to use when generating the classname. See
	 *          {@link PatternNamingScheme} for more details.
	 */
	public ClassNamingScheme create(final String namingScheme,
			final String namingPattern) throws MojoExecutionException {

		if (namingScheme.equals("simple")) {
			return new PatternNamingScheme("Parallel{c}IT", counter,
					featureFileNamingScheme);
