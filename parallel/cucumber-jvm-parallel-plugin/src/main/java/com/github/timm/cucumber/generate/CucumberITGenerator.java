...
/**
 * Generates Cucumber runner files using configuration from FileGeneratorConfig
 * containing parameters passed into the Maven Plugin configuration.
 */
...
public class CucumberITGenerator {
	...
	/**
	 * @param config
	 *          The configuration parameters passed to the Maven Mojo
	 * @param overriddenParameters
	 *          Parameters overridden from Cucumber options VM parameter
	 *          (-Dcucumber.options)
	 * @param classNamingScheme
	 *          The naming scheme to use for the generated class files
	 */
	...
	/**
	 * Sets the feature file location based on the given file. The full file path
	 * is trimmed to only include the featuresDirectory. E.g.
	...
	private String createFormatStrings() {
		...
		for (int i = 0; i < formatStrs.length; i++) {
			...
			sb.append(String.format("\"%s:%s/%s.%s\"", formatStr, config
					.getCucumberOutputDir().replace('\\', '/'), fileCounter, formatStr));
