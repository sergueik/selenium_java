 ...
 * 
 * Fork of
 * https://github.com/cucumber/cucumber-jvm/blob/master/core/src/main/java
 * /cucumber/runtime/RuntimeOptions.java
 */
public class RuntimeOptions {
	...
	public RuntimeOptions(List<String> argv) {
		...
		argv = new ArrayList<String>(argv); // in case the one passed in is
																				// unmodifiable.
		parse(argv);
	...
	private void parse(final List<String> args) {
		...
		while (!args.isEmpty()) {
			...
			if (arg.equals("--glue") || arg.equals("-g")) {
				...
				System.err
						.println("WARNING: Cucumber-JVM's --format option is deprecated. "
								+ "Please use --plugin instead.");
				addPluginName(args.remove(0));
			} else if (arg.equals("--no-dry-run") || arg.equals("--dry-run")
					|| arg.equals("-d")) {
				dryRun = !arg.startsWith("--no-");
			} else if (arg.equals("--no-strict") || arg.equals("--strict")
					|| arg.equals("-s")) {
