...
public class Correctness {
	...
	public void checkMaxRows() throws Exception {
		byte[] data = writeWorkbook(wb -> wb.newWorksheet("Worksheet 1")
				.value(Worksheet.MAX_ROWS - 1, 0, "test"));
	}

	@Test
	public void checkMaxCols() throws Exception {
		byte[] data = writeWorkbook(wb -> wb.newWorksheet("Worksheet 1").value(0,
				Worksheet.MAX_COLS - 1, "test"));
	}

	@Test(expected = IllegalArgumentException.class)
	public void exceedMaxRows() throws Exception {
		byte[] data = writeWorkbook(wb -> wb.newWorksheet("Worksheet 1")
				.value(Worksheet.MAX_ROWS, 0, "test"));
	}

	@Test(expected = IllegalArgumentException.class)
	public void negativeRow() throws Exception {
		byte[] data = writeWorkbook(
				wb -> wb.newWorksheet("Worksheet 1").value(-1, 0, "test"));
	}

	@Test(expected = IllegalArgumentException.class)
	public void exceedMaxCols() throws Exception {
		byte[] data = writeWorkbook(wb -> wb.newWorksheet("Worksheet 1").value(0,
				Worksheet.MAX_COLS, "test"));
	}

	@Test(expected = IllegalArgumentException.class)
	public void negativeCol() throws Exception {
		byte[] data = writeWorkbook(
				wb -> wb.newWorksheet("Worksheet 1").value(0, -1, "test"));
	}

	@Test(expected = IllegalArgumentException.class)
	public void notSupportedTypeCell() throws Exception {
		byte[] data = writeWorkbook(
				wb -> wb.newWorksheet("Worksheet 1").value(0, 0, new Object()));
	}
	...
	public void singleWorksheet() throws Exception {
		...
		// We truncate to milliseconds because JDK 9 gives microseconds, whereas
		// Excel format supports only milliseconds
		LocalDateTime localDateTimeValue = LocalDateTime.now()
				.truncatedTo(ChronoUnit.MILLIS);
		ZoneId timezone = ZoneId.of("Australia/Sydney");
		ZonedDateTime zonedDateValue = ZonedDateTime.ofInstant(
				dateValue.toInstant().truncatedTo(ChronoUnit.MILLIS), timezone);
		...
		assertThat(xws.getRow(i).getCell(i++).getStringCellValue())
				.isEqualTo(stringValue);
		assertThat(xws.getRow(i).getCell(i++).getDateCellValue())
				.isEqualTo(dateValue);
		// Check zoned timestamps have the same textual representation as the Dates
		...
		assertThat(DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(ZonedDateTime
				.ofInstant(xws.getRow(i).getCell(i++).getDateCellValue().toInstant(),
						ZoneId.systemDefault())))
								.isEqualTo(DateTimeFormatter.ISO_LOCAL_DATE_TIME
										.format(localDateTimeValue));
		assertThat(DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(ZonedDateTime
				.ofInstant(xws.getRow(i).getCell(i++).getDateCellValue().toInstant(),
						ZoneId.systemDefault()))).isEqualTo(
								DateTimeFormatter.ISO_LOCAL_DATE_TIME.format(zonedDateValue));
		assertThat(xws.getRow(i).getCell(i++).getNumericCellValue())
				.isEqualTo(doubleValue);
		assertThat(xws.getRow(i).getCell(i++).getNumericCellValue())
				.isEqualTo(intValue);
		assertThat(xws.getRow(i).getCell(i++).getNumericCellValue())
				.isEqualTo(longValue);
		assertThat(new BigDecimal(xws.getRow(i).getCell(i++).getRawValue()))
	...
	public void multipleWorksheets() throws Exception {
		...
		byte[] data = writeWorkbook(wb -> {
			...
			for (int i = 0; i < cfs.length; ++i) {
				...
				CompletableFuture<Void> cf = CompletableFuture.runAsync(() -> {
					...
					ws.formula(numRows + 1, 1,
							"=SUM(" + ws.range(1, 1, numRows, 1).toString() + ")");
					ws.formula(numRows + 1, 2,
							"=SUM(" + ws.range(1, 2, numRows, 2).toString() + ")");
					ws.formula(numRows + 1, 3,
							"=SUM(" + ws.range(1, 3, numRows, 3).toString() + ")");
					ws.formula(numRows + 1, 4,
							"=AVERAGE(" + ws.range(1, 4, numRows, 4).toString() + ")");
					ws.style(numRows + 1, 4).format("yyyy-MM-dd HH:mm:ss").set();
					ws.formula(numRows + 1, 5,
							"=AVERAGE(" + ws.range(1, 5, numRows, 5).toString() + ")");
					ws.style(numRows + 1, 5).format("yyyy-MM-dd").bold().italic()
							.fontColor(Color.RED).fontName("Garamond")
							.fontSize(new BigDecimal("14.5")).horizontalAlignment("center")
							.verticalAlignment("top").wrapText(true).set();
					ws.range(1, 0, numRows, numCols - 1).style().borderColor(Color.RED)
							.borderStyle("thick").shadeAlternateRows(Color.RED).set();
				});
				cfs[i] = cf;
			}
			try {
		...
		for (int i = 0; i < numWs; ++i) {
			...
			for (int j = 1; j <= numRows; ++j) {
				assertThat(xws.getRow(j).getCell(0).getStringCellValue())
						.isEqualTo("String value " + j);
			}
		}
	}
	...
	public void borders() throws Exception {
		byte[] data = writeWorkbook(wb -> {
			...
			ws.style(1, 3).borderStyle(BorderSide.BOTTOM, "thick")
					.borderColor(BorderSide.BOTTOM, Color.RED).set();
			ws.style(1, 4).borderStyle(BorderSide.TOP, "thin").set();
			ws.style(1, 5).borderStyle(BorderSide.LEFT, "thin")
					.borderStyle(BorderSide.BOTTOM, "thick").set();
			ws.style(1, 6).borderStyle(BorderSide.RIGHT, "thin").set();
			for (int col = 1; col < 10; ++col) {
				ws.style(2, col).borderStyle(BorderSide.LEFT, "thin")
		...
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getTopBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getLeftBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getBottomBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getRightBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.THIN);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.THIN);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.THIN);
		assertThat(xws.getRow(1).getCell(1).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.THIN);

		byte[] red = new byte[3];
		red[0] = -1;
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getTopBorderXSSFColor()
				.getRGB()).isEqualTo(red);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getLeftBorderXSSFColor()
				.getRGB()).isEqualTo(red);
		assertThat(xws.getRow(1).getCell(2).getCellStyle()
				.getBottomBorderXSSFColor().getRGB()).isEqualTo(red);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getRightBorderXSSFColor()
				.getRGB()).isEqualTo(red);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.THICK);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.THICK);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.THICK);
		assertThat(xws.getRow(1).getCell(2).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.THICK);

		assertThat(xws.getRow(1).getCell(3).getCellStyle().getTopBorderXSSFColor())
				.isNull();
		assertThat(xws.getRow(1).getCell(3).getCellStyle().getLeftBorderXSSFColor())
				.isNull();
		assertThat(xws.getRow(1).getCell(3).getCellStyle()
				.getBottomBorderXSSFColor().getRGB()).isEqualTo(red);
		assertThat(
				xws.getRow(1).getCell(3).getCellStyle().getRightBorderXSSFColor())
						.isNull();
		assertThat(xws.getRow(1).getCell(3).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(3).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(3).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.THICK);
		assertThat(xws.getRow(1).getCell(3).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.NONE);

		assertThat(xws.getRow(1).getCell(4).getCellStyle().getTopBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(4).getCellStyle().getLeftBorderXSSFColor())
				.isNull();
		assertThat(
				xws.getRow(1).getCell(4).getCellStyle().getBottomBorderXSSFColor())
						.isNull();
		assertThat(
				xws.getRow(1).getCell(4).getCellStyle().getRightBorderXSSFColor())
						.isNull();
		assertThat(xws.getRow(1).getCell(4).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.THIN);
		assertThat(xws.getRow(1).getCell(4).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(4).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(4).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.NONE);

		assertThat(xws.getRow(1).getCell(5).getCellStyle().getTopBorderXSSFColor())
				.isNull();
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getLeftBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getBottomBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(
				xws.getRow(1).getCell(5).getCellStyle().getRightBorderXSSFColor())
						.isNull();
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.THIN);
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.THICK);
		assertThat(xws.getRow(1).getCell(5).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.NONE);

		assertThat(xws.getRow(1).getCell(6).getCellStyle().getTopBorderXSSFColor())
				.isNull();
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getLeftBorderXSSFColor())
				.isNull();
		assertThat(
				xws.getRow(1).getCell(6).getCellStyle().getBottomBorderXSSFColor())
						.isNull();
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getRightBorderColor())
				.isEqualTo(IndexedColors.BLACK.index);
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getBorderTopEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getBorderLeftEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getBorderBottomEnum())
				.isEqualTo(BorderStyle.NONE);
		assertThat(xws.getRow(1).getCell(6).getCellStyle().getBorderRightEnum())
				.isEqualTo(BorderStyle.THIN);

		for (int col = 1; col < 10; ++col) {
			assertThat(
					xws.getRow(2).getCell(col).getCellStyle().getTopBorderXSSFColor())
							.isNull();
			assertThat(xws.getRow(2).getCell(col).getCellStyle().getLeftBorderColor())
					.isEqualTo(IndexedColors.BLACK.index);
			assertThat(
					xws.getRow(2).getCell(col).getCellStyle().getBottomBorderColor())
							.isEqualTo(IndexedColors.BLACK.index);
			assertThat(
					xws.getRow(2).getCell(col).getCellStyle().getRightBorderXSSFColor())
							.isNull();
			assertThat(xws.getRow(2).getCell(col).getCellStyle().getBorderTopEnum())
					.isEqualTo(BorderStyle.NONE);
			assertThat(xws.getRow(2).getCell(col).getCellStyle().getBorderLeftEnum())
					.isEqualTo(BorderStyle.THIN);
			assertThat(
					xws.getRow(2).getCell(col).getCellStyle().getBorderBottomEnum())
							.isEqualTo(BorderStyle.THIN);
			assertThat(xws.getRow(2).getCell(col).getCellStyle().getBorderRightEnum())
					.isEqualTo(BorderStyle.NONE);
		}
	}
	...
	public void listValidations() throws Exception {
		...
		byte[] data = writeWorkbook(wb -> {
			...
			ListDataValidation listDataValidation = ws.range(0, 0, 100, 0)
					.validateWithList(listRange);
			listDataValidation.allowBlank(false).error(errMsg).errorTitle(errTitle)
					.errorStyle(DataValidationErrorStyle.WARNING).showErrorMessage(true);
		});
		...
		assertThat(dataValidation.getRegions().getCellRangeAddresses().length)
				.isEqualTo(1);

		CellRangeAddress cellRangeAddress = dataValidation.getRegions()
				.getCellRangeAddress(0);
		...
		DataValidationConstraint validationConstraint = dataValidation
				.getValidationConstraint();
		assertThat(validationConstraint.getFormula1().toLowerCase())
				.isEqualToIgnoringCase("Lists!A1:A2");
	}
	...
	public void canHideSheet() throws IOException {
		...
		assertThat(xwb.getSheetVisibility(2))
				.isEqualTo(SheetVisibility.VERY_HIDDEN);
	...
	public void canProtectCells() throws Exception {
		...
		byte[] data = writeWorkbook(wb -> {
			...
			ws.value(0, 1, "val1");
			ws.style(0, 1).protectionOption(ProtectionOption.LOCKED, true).set();

			ws.value(0, 2, "val2");
			ws.style(0, 2).protectionOption(ProtectionOption.HIDDEN, true).set();

			ws.value(0, 3, "val3");
			ws.style(0, 3).protectionOption(ProtectionOption.LOCKED, false).set();

			ws.value(0, 4, "val4");
			ws.style(0, 4).protectionOption(ProtectionOption.LOCKED, false)
		...
		assertFalse("Cell (0, 1) should NOT be hidden",
				xws.getRow(0).getCell(1).getCellStyle().getHidden());
		assertTrue("Cell (0, 1) should be locked",
				xws.getRow(0).getCell(1).getCellStyle().getLocked());

		assertTrue("Cell (0, 2) should be locked (by default)",
				xws.getRow(0).getCell(2).getCellStyle().getLocked());
		assertTrue("Cell (0, 2) should be hidden",
				xws.getRow(0).getCell(2).getCellStyle().getHidden());

		assertFalse("Cell (0, 3) should NOT be hidden",
				xws.getRow(0).getCell(3).getCellStyle().getHidden());
		assertFalse("Cell (0, 3) should NOT be locked",
				xws.getRow(0).getCell(3).getCellStyle().getLocked());

		assertFalse("Cell (0, 4) should NOT be locked",
				xws.getRow(0).getCell(4).getCellStyle().getLocked());
		assertFalse("Cell (0, 4) should NOT be hidden",
				xws.getRow(0).getCell(4).getCellStyle().getHidden());
	}
}
