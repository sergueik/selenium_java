...
public class Workbook {
	...
	public Workbook(OutputStream os, String applicationName,
		...
		if (applicationVersion != null
				&& !applicationVersion.matches("\\d{1,2}\\.\\d{1,4}")) {
			throw new IllegalArgumentException(
					"Application version must be of the form XX.YYYY");
		}
	...
	public void finish() throws IOException {
		...
		writeFile("[Content_Types].xml", w -> {
			...
			for (Worksheet ws : worksheets) {
				w.append("<Override PartName=\"/xl/worksheets/sheet")
						.append(getIndex(ws)).append(
		...
		writeFile("docProps/app.xml",
				w -> w
						...
						.appendEscaped(applicationName).append("</Application>")
						.append(applicationVersion == null ? ""
								: ("<AppVersion>" + applicationVersion + "</AppVersion>"))
						.append("</Properties>"));
		writeFile("docProps/core.xml", w -> w
				.append(
				...
				.append("Z</dcterms:created><dc:creator>")
				.appendEscaped(applicationName)
				.append("</dc:creator></cp:coreProperties>"));
		writeFile("_rels/.rels", w -> w.append(
		...
		writeFile("xl/_rels/workbook.xml.rels", w -> {
			...
			for (Worksheet ws : worksheets) {
				w.append("<Relationship Id=\"rId").append(getIndex(ws) + 2)
						.append("\" Target=\"worksheets/sheet").append(getIndex(ws)).append(
	...
	private void writeWorkbookFile() throws IOException {
		writeFile("xl/workbook.xml", w -> {
			w.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<workbook "
					+ "xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\" "
					+ "xmlns:r=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships\">"
					+ "<workbookPr date1904=\"false\"/>" + "<bookViews>"
	...
	private void writeWorkbookSheet(Writer w, Worksheet ws) throws IOException {
		w.append("<sheet name=\"").appendEscaped(ws.getName())
				.append("\" r:id=\"rId").append(getIndex(ws) + 2)
	 ...
	 * @throws IOException If an I/O error occurs.
	 */
	void writeFile(String name, ThrowingConsumer<Writer> consumer)
			throws IOException {
		synchronized (os) {
	 ...
	 */
	int mergeAndCacheStyle(int currentStyle, String numberingFormat, Font font,
			Fill fill, Border border, Alignment alignment, Protection protection) {
		return styleCache.mergeAndCacheStyle(currentStyle, numberingFormat, font,
				fill, border, alignment, protection);
	}
	...
	public Worksheet newWorksheet(String name) {
		// Replace chars forbidden in worksheet names (backslahses and colons) by
		// dashes
		...
		synchronized (worksheets) {
			...
			int number = 1;
			Set<String> names = worksheets.stream().map(Worksheet::getName)
