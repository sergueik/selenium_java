...
public class StyleSetter {
	...
	public StyleSetter borderStyle(String borderStyle) {
		...
		EnumSet.of(BorderSide.TOP, BorderSide.LEFT, BorderSide.BOTTOM,
				BorderSide.RIGHT).forEach(side -> {
					borderElement(side,
							border.elements.get(side).updateStyle(borderStyle));
				});
		return this;
	}
	...
	public StyleSetter borderColor(String borderColor) {
		...
		EnumSet.of(BorderSide.TOP, BorderSide.LEFT, BorderSide.BOTTOM,
				BorderSide.RIGHT).forEach(side -> {
					borderElement(side,
							border.elements.get(side).updateColor(borderColor));
				});
		return this;
	}
	...
	public void set() {
		...
		Font font;
		if (bold || italic || fontColor != null || fontName != null
				|| fontSize != null) {
		...
		Map<Integer, Integer> newStyles = currentStyles.stream()
				.collect(
						Collectors.toMap(Function.identity(),
								s -> range.getWorksheet().getWorkbook().mergeAndCacheStyle(s,
										valueFormatting, font, fill, border, alignment,
										protection)));
		...
		if (alternateShadingFillColor != null) {
			range
					.shadeAlternateRows(Fill.fromColor(alternateShadingFillColor, false));
		}
	}
}
