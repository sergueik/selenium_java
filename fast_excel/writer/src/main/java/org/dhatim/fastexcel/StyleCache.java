...
final class StyleCache {
	...
	private static <T> int cacheStuff(ConcurrentMap<T, Integer> cache, T t,
			Function<T, Integer> indexFunction) {
		Integer index = cache.get(t);
		if (index == null) {
			// Need to synchronize to get a valid new index (the index function
	...
	int mergeAndCacheStyle(int currentStyle, String numberingFormat, Font font,
		...
		Style original = styles.entrySet().stream()
				.filter(e -> e.getValue().equals(currentStyle)).map(Entry::getKey)
				.findFirst().orElse(null);
		Style s = new Style(original, cacheValueFormatting(numberingFormat),
				cacheFont(font), cacheFill(fill), cacheBorder(border), alignment,
				protection);
	 ...
	 */
	private static <T> void writeCache(Writer w, Map<T, Integer> cache,
			String name, ThrowingConsumer<Entry<T, Integer>> consumer)
			throws IOException {
		w.append('<').append(name).append(" count=\"").append(cache.size())
				.append("\">");
	...
	void write(Writer w) throws IOException {
		...
		writeCache(w, valueFormattings, "numFmts",
				e -> w.append("<numFmt numFmtId=\"").append(e.getValue())
						.append("\" formatCode=\"").append(e.getKey()).append("\"/>"));
