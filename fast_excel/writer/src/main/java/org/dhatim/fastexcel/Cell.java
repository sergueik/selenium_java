...
class Cell {
	...
	void write(Writer w, int r, int c) throws IOException {
		if (value != null || style != 0) {
			w.append("<c r=\"").append(Range.colToString(c)).append(r + 1)
					.append('\"');
			if (style != 0) {
				w.append(" s=\"").append(style).append('\"');
			}
			if (value != null && !(value instanceof Formula)) {
				w.append(" t=\"").append((value instanceof CachedString) ? 's'
						: (value instanceof Boolean ? 'b' : 'n')).append('\"');
			}
			w.append(">");
			if (value instanceof Formula) {
				w.append("<f>").append(((Formula) value).getExpression())
						.append("</f>");
			} else if (value != null) {
				w.append("<v>");
	...
	void setValue(Workbook wb, Object v) {
		if (v instanceof String) {
			...
			value = TimestampUtil.convertDate(Date.from(
			...
			throw new IllegalArgumentException(
					"No supported cell type for " + v.getClass());
		}
	}
