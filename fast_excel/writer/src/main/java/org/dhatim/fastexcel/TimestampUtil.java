...
public final class TimestampUtil {
	 ...
	 */
	private static final LocalDateTime EPOCH_1900 = LocalDateTime.of(1900, 1, 1,
			0, 0, 0, 0);
	...
	private static Double convertDate(Date date, ZoneId timezone) {
		LocalDateTime ldt = LocalDateTime
				.ofInstant(Instant.ofEpochMilli(date.getTime()), timezone);
		Duration duration = Duration.between(EPOCH_1900, ldt);
		return duration.getSeconds() / 86400.0
				+ duration.getNano() / 86400000000000.0 + 2; // 2 to compensate for
																											// Excel bugs
	}
