...
public class Range {
	...
	Range(Worksheet worksheet, int top, int left, int bottom, int right) {
		...
		// Check limits
		if (top < 0 || top >= Worksheet.MAX_ROWS || bottom < 0
				|| bottom >= Worksheet.MAX_ROWS) {
			throw new IllegalArgumentException();
		}
		if (left < 0 || left >= Worksheet.MAX_COLS || right < 0
				|| right >= Worksheet.MAX_COLS) {
			throw new IllegalArgumentException();
		}
	...
	public boolean equals(Object obj) {
		...
		if (obj != null && obj.getClass() == this.getClass()) {
			...
			result = Objects.equals(worksheet, other.worksheet)
					&& Objects.equals(top, other.top) && Objects.equals(left, other.left)
	...
	public String toString() {
		return colToString(left) + Integer.toString(top + 1) + ':'
				+ colToString(right) + Integer.toString(bottom + 1);
	}
	...
	public ListDataValidation validateWithList(Range listRange) {
		ListDataValidation listDataValidation = new ListDataValidation(this,
				listRange);
