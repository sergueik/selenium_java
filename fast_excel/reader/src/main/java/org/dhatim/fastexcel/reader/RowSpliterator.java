...
class RowSpliterator implements Spliterator<Row> {
	...
	public RowSpliterator(ReadableWorkbook workbook, InputStream inputStream)
	...
	private Row next() throws XMLStreamException {
		...
		while (r.goTo(() -> r.isStartElement("c") || r.isEndElement("row"))) {
			...
			if ("inlineStr".equals(type)) {
				...
				String f = null;
				String rv = null;
				while (r.goTo(() -> r.isStartElement("is") || r.isEndElement("c")
						|| r.isStartElement("f"))) {
				...
				String f = null;
				String rv = null;
				while (r.goTo(() -> r.isStartElement("v") || r.isEndElement("c")
						|| r.isStartElement("f"))) {
		...
		return new Row(rowIndex, physicalCellCount,
				cells.toArray(new Cell[cells.size()]));
	}
