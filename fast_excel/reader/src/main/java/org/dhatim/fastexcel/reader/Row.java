...
public class Row implements Iterable<Cell> {
	...
	public Cell getCell(int index) {
		if (index < 0 || index >= cells.length) {
			throw new IndexOutOfBoundsException("row-index: " + rowNum + ", index: "
	...
	public Cell getCell(CellAddress address) {
		if (rowNum != address.getRow()) {
			throw new IllegalArgumentException("The given address " + address
					+ " concerns another row (" + rowNum + ")");
		}
	...
	public Optional<Cell> getOptionalCell(int index) {
		return index < 0 || index >= cells.length ? Optional.empty()
				: Optional.ofNullable(cells[index]);
	}

	public Optional<Cell> getFirstNonEmptyCell() {
		return stream().filter(Objects::nonNull)
				.filter(cell -> !cell.getText().isEmpty()).findFirst();
	}
