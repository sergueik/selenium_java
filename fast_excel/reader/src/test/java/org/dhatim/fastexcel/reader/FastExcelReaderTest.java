...
public class FastExcelReaderTest {
	...
	private static final class RowDates {
		...
		public boolean equals(Object obj) {
			...
			if (obj instanceof RowDates) {
				...
				return rowNum == other.rowNum && Objects.equals(date1, other.date1)
	...
	private static final Logger LOGGER = Logger
			.getLogger(FastExcelReaderTest.class.getName());
	...
	public void testDates() throws IOException, OpenXML4JException {
		...
		try (InputStream inputStream = open(EXCEL_DATES);
			...
			try (Stream<Row> stream = fworkbook.getFirstSheet().openStream()) {
				stream.forEach(row -> {
					values.add(
							new RowDates(row.getRowNum(), row.getCell(0).asDate().toString(),
		...
		try (InputStream inputStream = open(EXCEL_DATES);
			...
			for (org.apache.poi.ss.usermodel.Row row : workbook.getSheetAt(0)) {
				wvalues.add(new RowDates(row.getRowNum() + 1,
	...
	@ParameterizedTest
	@ValueSource(strings = { "/xlsx/AutoFilter.xlsx",
			"/xlsx/calendar_stress_test.xlsx", "/xlsx/cell_style_simple.xlsx",
			"/xlsx/comments_stress_test.xlsx", "/xlsx/custom_properties.xlsx",
			"/xlsx/dates.xlsx", "/xlsx/defined_names_simple.xlsx",
			"/xlsx/ErrorTypes.xlsx", "/xlsx/formula_stress_test.xlsx",
			"/xlsx/formulae_test_simple.xlsx", "/xlsx/hyperlink_no_rels.xlsx",
			"/xlsx/hyperlink_stress_test_2011.xlsx", "/xlsx/interview.xlsx",
			"/xlsx/issue.xlsx",
			// "/xlsx/large_strings.xlsx",
			"/xlsx/LONumbers-2010.xlsx", "/xlsx/LONumbers-2011.xlsx",
			"/xlsx/LONumbers.xlsx", "/xlsx/merge_cells.xlsx",
			"/xlsx/mixed_sheets.xlsx", "/xlsx/named_ranges_2011.xlsx",
			"/xlsx/number_format_entities.xlsx", "/xlsx/phonetic_text.xlsx",
			"/xlsx/pivot_table_named_range.xlsx", "/xlsx/rich_text_stress.xlsx",
			"/xlsx/RkNumber.xlsx", "/xlsx/smart_tags_2007.xlsx", "/xlsx/sushi.xlsx",
	...
	public void testFile(String file) {
		...
		try (InputStream inputStream = open(file);
				InputStream inputStream2 = open(file)) {
					...
					try (Stream<Row> data = sheetDef.openStream()) {
						...
						while (rowIt.hasNext()) {
							...
							assertThat(row.getCellCount()).as("logical cell")
									.isEqualTo(expected.getLastCellNum() == -1 ? 0
											: expected.getLastCellNum());

							for (int i = 0; i < row.getCellCount(); i++) {
								Cell cell = row.getCell(i);
								org.apache.poi.ss.usermodel.Cell expCell = expected.getCell(i);

								assertThat(cell == null).as("cell defined " + i)
										.isEqualTo(expCell == null);
								if (cell != null) {
									String cellAddr = cell.getAddress().toString();
									assertThat(toCode(cell.getType()))
											.as("cell type code " + cellAddr)
						...
						throw new RuntimeException(
								"On sheet " + sheetDef.getId() + " " + sheetDef.getName(), e);
					}
				}
