...
public class BenchmarksTest {
	...
	private static class SheetContentHandler
	...
	public long apachePoi() throws IOException {
		try (Workbook wb = WorkbookFactory.create(openResource(FILE))) {
			...
			long sum = StreamSupport
					.stream(Spliterators.spliteratorUnknownSize(sheet.rowIterator(),
							Spliterator.ORDERED), false)
					.skip(1).mapToLong(r -> (long) r.getCell(0).getNumericCellValue())
					.sum();
	...
	public long fastExcelReader() throws IOException {
		try (InputStream is = openResource(FILE);
			...
			try (Stream<Row> rows = sheet.openStream()) {
				long sum = rows.skip(1)
						.mapToLong(r -> r.getCell(0).asNumber().longValue()).sum();
				assertEquals(RESULT, sum);
				return sum;
			}
		}
	...
	@Benchmark
	public int streamingApachePoi()
			throws IOException, OpenXML4JException, SAXException {
		try (OPCPackage pkg = OPCPackage.open(openResource(FILE))) {
			ReadOnlySharedStringsTable strings = new ReadOnlySharedStringsTable(pkg);
			XSSFReader reader = new XSSFReader(pkg);
			StylesTable styles = reader.getStylesTable();
			XSSFReader.SheetIterator iterator = (XSSFReader.SheetIterator) reader
					.getSheetsData();
			int sheetIndex = 0;
	...
	public long monitorjbl() throws IOException {
		long sum = 0;
		try (InputStream is = openResource(FILE);
				org.apache.poi.ss.usermodel.Workbook workbook = StreamingReader
						.builder().open(is)) {
	...
	public void benchmarks() throws RunnerException {
		...
		Options options = new OptionsBuilder().include(foo)
				.mode(Mode.SingleShotTime).warmupIterations(1).warmupBatchSize(1)
				.measurementIterations(1).threads(1).forks(1)
				.timeUnit(TimeUnit.MILLISECONDS).shouldFailOnError(true)
				.resultFormat(ResultFormatType.CSV).result("jmh.csv").build();
