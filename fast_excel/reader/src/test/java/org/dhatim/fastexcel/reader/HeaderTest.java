...
public class HeaderTest {
	...
	public void testInsufficientBytes() {
		...
		assertThatThrownBy(() -> ReadableWorkbook.isOLE2Header(bytes))
				.isInstanceOf(IllegalArgumentException.class);
		assertThatThrownBy(() -> ReadableWorkbook.isOOXMLZipHeader(bytes))
				.isInstanceOf(IllegalArgumentException.class);
	}
	...
	private static byte[] readFirstBytes(String name, int length)
			throws IOException {
	...
	private static int readNBytes(InputStream is, byte[] b, int off, int len)
			throws IOException {
