...
class WindowsProcessesService extends AbstractProcessesService {
	...
	private void processLine(String dataLine,
		...
		if (processMap != null) {
			...
			if (CREATIONDATE_PROPNAME.equals(dataStringInfo[0].trim())) {
				processMap.put("start_datetime", ProcessesUtils
						.parseWindowsDateTimeToFullDate(dataStringInfo[1].trim()));
			}
		}
	}
	...
	protected String getProcessesData(String name) {
		...
		if (name != null) {
			return getWmi4Java().VBSEngine()
					.properties(Arrays.asList(CAPTION_PROPNAME, PROCESSID_PROPNAME,
							NAME_PROPNAME, USERMODETIME_PROPNAME, COMMANDLINE_PROPNAME,
	...
	protected JProcessesResponse kill(int pid) {
		...
		if (ProcessesUtils.executeCommandAndGetCode("taskkill", "/PID",
				String.valueOf(pid), "/F") == 0) {
	...
	private static String normalizeValue(String origKey, String origValue) {
		...
		if (VIRTUALSIZE_PROPNAME.equals(origKey)
				|| WORKINGSETSIZE_PROPNAME.equals(origKey)) {
	...
	private void fillExtraProcessData() {
		String perfData = getWmi4Java().VBSEngine().getRawWMIObjectOutput(
	...
	private static String checkAndGetDataInLine(String dataName,
			String dataLine) {
