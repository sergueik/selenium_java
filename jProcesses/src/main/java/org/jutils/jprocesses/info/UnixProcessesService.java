...
class UnixProcessesService extends AbstractProcessesService {
	...
	private static final int PS_FULL_COMMAND_SIZE = PS_FULL_COMMAND
			.split(",").length;
	...
	protected List<Map<String, String>> parseList(String rawData) {
		...
		for (final String dataLine : dataStringLines) {
			...
			if (line.startsWith("PID")) {
				...
				index++; // Skip weekday
				String longDate = elements[index++] + " " + elements[index++] + " "
				...
				try {
					...
					System.err.println("Failed formatting date from ps: " + longDate
							+ ", using \"01/01/2000 00:00:00\"");
				}
	...
	protected String getProcessesData(String name) {
		if (name != null) {
			if (OSDetector.isLinux()) {
				return ProcessesUtils.executeCommand("ps", "-o", PS_COLUMNS, "-C",
						name);
			} else {
	...
	protected JProcessesResponse kill(int pid) {
		...
		if (ProcessesUtils.executeCommandAndGetCode("kill", "-9",
				String.valueOf(pid)) == 0) {
	...
	protected JProcessesResponse killGracefully(int pid) {
		JProcessesResponse response = new JProcessesResponse();
		if (ProcessesUtils.executeCommandAndGetCode("kill", "-15",
				String.valueOf(pid)) == 0) {
			response.setSuccess(true);
		}
		return response;
	...
	public JProcessesResponse changePriority(int pid, int priority) {
		JProcessesResponse response = new JProcessesResponse();
		if (ProcessesUtils.executeCommandAndGetCode("renice",
				String.valueOf(priority), "-p", String.valueOf(pid)) == 0) {
			response.setSuccess(true);
		}
		return response;
	}
	...
	public ProcessInfo getProcess(int pid, boolean fastMode) {
		...
		List<Map<String, String>> processList = parseList(ProcessesUtils
				.executeCommand("ps", "-o", PS_COLUMNS, "-p", String.valueOf(pid)));
	...
	private static void loadFullCommandData(
			List<Map<String, String>> processesDataList) {
		Map<String, String> commandsMap = new HashMap<String, String>();
		String data = ProcessesUtils.executeCommand("ps", "-e", "-o",
				PS_FULL_COMMAND);
