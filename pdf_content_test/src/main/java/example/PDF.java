...
import example.matchers.ContainsExactText;
import example.matchers.ContainsText;
import example.matchers.ContainsTextCaseInsensitive;
import example.matchers.DoesNotContainExactText;
...
public class PDF {
	...
	public PDF(File pdfFile, int startPage, int endPage) throws IOException {
		this(pdfFile.getAbsolutePath(),
				Files.readAllBytes(Paths.get(pdfFile.getAbsolutePath())), startPage,
				endPage);
	}
	...
	public PDF(InputStream inputStream, int startPage, int endPage)
			throws IOException {
