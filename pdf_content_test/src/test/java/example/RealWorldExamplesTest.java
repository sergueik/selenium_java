...
import static example.PDF.containsText;
...
public class RealWorldExamplesTest {
	...
	public void bankStatement() throws Exception {
		...
		assertThat(pdf,
				containsText("Дата Плательщик / Получатель Операция Сумма (RUB)"));
		assertThat(pdf, containsText(
				"18.06.2015 Solntsev Andrei \t 40820810590550000146 \t сюда. Интернет-банк -1.00"));
		assertThat(pdf, containsText(
				"18.06.2015 Solntsev Andrei \t 40820810590550000146 \t туда. Интернет-банк 1.00"));
	...
	public void bankTransaction() throws Exception {
		PDF pdf = new PDF(
				getClass().getClassLoader().getResource("transaction.pdf"));
	...
	public void someWeirdPdf_singlePage() throws IOException {
		PDF pdf = new PDF(
				getClass().getClassLoader().getResource("PO_11179_1520159715.pdf"));
	...
	public void someWeirdPdf_multiplePages() throws IOException {
		PDF pdf = new PDF(
				getClass().getClassLoader().getResource("PO_US-14602_1521108736.pdf"));
	...
	public void someWeirdPdf_takeTheFirstPage() throws IOException {
		PDF pdf = new PDF(
				...
				1, 1);
		assertThat(pdf, containsText("Purchase Order #US-14602"));
		assertThat(pdf, not(containsText(
				"STANDARD SELECTICA CONTRACT PERFORMANCE MANAGEMENT REPORTS")));
	}
	...
	public void someWeirdPdf_takeTheSecondPage() throws IOException {
		...
		assertThat(pdf, not(containsText(
				"All Employment Contracts are grouped by Employee Status")));
	}
