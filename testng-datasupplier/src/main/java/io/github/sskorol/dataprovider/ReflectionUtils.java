...
public final class ReflectionUtils {
	...
	public static Method getMethod(final Class<?> targetClass,
		...
		final Tuple2<String, Class<?>[]> metaData = StreamEx
				.of(targetClass.getMethods())
				.map(m -> Tuple.of(m, m.getDeclaredAnnotation(DataSupplier.class)))
				.filter(
						t -> Objects.nonNull(t._2) && (t._2.name().equals(targetMethodName)
	...
	public static DataSupplier getDataSupplierAnnotation(
			final Class<?> targetClass, final String targetMethodName) {
		return Try.of(() -> getMethod(targetClass, targetMethodName))
				.map(m -> m.getDeclaredAnnotation(DataSupplier.class))
				.filter(Objects::nonNull).getOrElse((DataSupplier) null);
	}

	public static Object invoke(final Method method,
			final Supplier<Object[]> argsSupplier) {
		return on(method.getDeclaringClass()).create()
				.call(method.getName(), argsSupplier.get()).get();
	}
}
