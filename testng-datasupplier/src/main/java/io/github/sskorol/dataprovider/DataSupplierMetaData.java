...
class DataSupplierMetaData {
	...
	DataSupplierMetaData(final ITestContext context, final Method testMethod) {
		...
		final Class<?> dataSupplierClass = testAnnotation
				.dataProviderClass() != Object.class
		...
		this.isExtractable = ofNullable(
				dataSupplierMethod.getDeclaredAnnotation(DataSupplier.class))
						.map(DataSupplier::extractValues).orElse(false);
	}

	public Iterator<Object[]> transform() {
		final StreamEx<?> wrappedDataSupplierReturnValue = Match(
				obtainReturnValue()).of(Case($(isNull()), () -> {
					throw new IllegalArgumentException(format(
							...
							testMethod.getName()));
				}), Case($(instanceOf(Collection.class)),
						d -> StreamEx.of((Collection<?>) d)),
						Case($(instanceOf(Object[].class)), d -> StreamEx.of((Object[]) d)),
						Case($(instanceOf(double[].class)),
								d -> DoubleStreamEx.of((double[]) d).boxed()),
						Case($(instanceOf(int[].class)),
								d -> IntStreamEx.of((int[]) d).boxed()),
						Case($(instanceOf(long[].class)),
								d -> LongStreamEx.of((long[]) d).boxed()),
						Case($(instanceOf(Stream.class)), d -> StreamEx.of((Stream<?>) d)),
						Case($(instanceOf(Tuple.class)),
								d -> StreamEx.of(((Tuple) d).toSeq().toJavaArray())),
						Case($(), d -> StreamEx.of(d)));

		return isExtractable
				? singletonList(wrappedDataSupplierReturnValue.toArray()).iterator()
				: wrappedDataSupplierReturnValue.map(ob -> new Object[] { ob })
						.iterator();
	}

	private Object obtainReturnValue() {
		return invoke(dataSupplierMethod,
				() -> stream(dataSupplierMethod.getParameterTypes())
						.map(t -> Match((Class) t).of(
								Case($(ITestContext.class), () -> context),
								Case($(Method.class), () -> testMethod), Case($(), () -> null)))
						.toArray());
	}
}
