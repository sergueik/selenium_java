...
public class CheckOut_YouCart_Page {
	...
	public static WebElement Product_Detail_Info(WebDriver driver, int row,
			int col) {
		WebElement ShoppingCartTable = driver.findElement(By
				.className("checkout_cart"));
		List<WebElement> tableRows = ShoppingCartTable.findElements(By
				.tagName("tr"));
		List<WebElement> rowElements = tableRows.get(row).findElements(
				By.tagName("td"));
	...
	public static String getProductName(WebDriver driver) {
		...
		return Product_Detail_Info(driver, shoppingCart_productInfo_Row,
				shoppingCart_nameCol).getText();
	}

	public static double getProductPrice(WebDriver driver) {
		return Double.parseDouble(Product_Detail_Info(driver, 1, 1).getText()
				.replace("$", ""));
	}
