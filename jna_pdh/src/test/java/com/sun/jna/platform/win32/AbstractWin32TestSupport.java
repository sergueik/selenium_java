...
public abstract class AbstractWin32TestSupport extends AbstractPlatformTestSupport {
	 ...
	 * themselves. This check is in order where APIs are WIN32 API
	 * <U>functions</U> since these are C functions - which means no overloading
	 * is possible.
	 *
	 * @param ifc
	 *            The interface (not checked) class to test
	...
	/**
	 * Checks if there are methods with the same name - regardless of the
	 * signature
	 *
	 * @param ifc
	 *            The interface (not checked) class to test
	 ...
	 * <B>Note:</B> if the error code is {@link WinError#ERROR_SUCCESS} then an
	 * <I>&quot;unknown reason code&quot;</I> is reported
	 * 
	 * @param message
	 *            Message to display if call failed
	 * @param result
	 *            The API call result
	 */
	 ...
	 * 
	 * @param message
	 *            Message to display if code is an error
	 * @param statusCode
	 *            Status code to check
	 * @param showHex
	 *            If status code is not error success then show it as HEX
	 */
	 ...
	 * invokes {@link Kernel32#GetLastError()} in order to display the error
	 * code
	 * 
	 * @param message
	 *            Message to display if bad handle
	 * @param handle
	 *            The {@link HANDLE} to test
	...
	public static final boolean isEnglishLocale = systemLCID.intValue() == 0x409 // en_US
	...
	public static void killProcessByName(String filename) {
		...
		while (hRes) {
			...
			if (imageName.equalsIgnoreCase(filename)) {
				HANDLE hProcess = Kernel32.INSTANCE.OpenProcess(Kernel32.PROCESS_TERMINATE, false,
						process.th32ProcessID.intValue());
				if (hProcess != null) {
