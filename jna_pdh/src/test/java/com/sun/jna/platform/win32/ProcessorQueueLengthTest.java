...
public class ProcessorQueueLengthTest extends AbstractWin32TestSupport {
	...
	private static String makeCounterPath(Pdh pdh,
		...
		assertThat("Unexpected status code: 0x" + Integer.toHexString(status),
				status, is(PdhMsg.PDH_MORE_DATA));

		DWORD bufSize = pcchBufferSize.getValue();
		int numChars = bufSize.intValue();
		assertThat("Bad required buffer size: " + numChars, numChars,
				greaterThan(0));

		char[] szFullPathBuffer = new char[numChars + 1 /* the \0 */];
		pcchBufferSize.setValue(new DWORD(szFullPathBuffer.length));
		assertThat("Error in PdhMakeCounterPath", pdh
	...
	public void test() {
		...
		assertThat("Error in PdhOpenQuery", pdh.PdhOpenQuery(null, null, ref),
				is(WinError.ERROR_SUCCESS));

		HANDLE hQuery = ref.getValue();
		try {
			ref.setValue(null);
			assertThat("Error in PdhAddEnglishCounter",
			...
			try {
				...
				assertThat("Error in PdhCollectQueryData call",
						pdh.PdhCollectQueryData(hQuery), is(WinError.ERROR_SUCCESS));
				DWORDByReference lpdwType = new DWORDByReference();
				PDH_RAW_COUNTER rawCounter = new PDH_RAW_COUNTER();
				assertThat("Error in PdhGetRawCounterValue",
						pdh.PdhGetRawCounterValue(hCounter, lpdwType, rawCounter),
						is(WinError.ERROR_SUCCESS));
				assertThat("unexected Counter data status", rawCounter.CStatus,
						is(WinError.ERROR_SUCCESS));
				showRawCounterData(System.out, counterName, rawCounter);
			} finally {
				assertThat("Error in PdhRemoveCounter", pdh.PdhRemoveCounter(hCounter),
						is(WinError.ERROR_SUCCESS));
			}
		} finally {
			assertThat("Error in PdhCloseQuery", pdh.PdhCloseQuery(hQuery),
					is(WinError.ERROR_SUCCESS));
		}
	}
}
