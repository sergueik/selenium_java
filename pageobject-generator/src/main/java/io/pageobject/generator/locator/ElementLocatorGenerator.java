...
public class ElementLocatorGenerator {
	...
	public Locator generate(GeneratorContext context) {
		...
		if (locatorPartGenerators.size() <= 1) {
			...
			parts.add(locatorPartGenerators.lastElement().lastPart(context,
					locatorPartGenerators.size() - 1));
		}

		LocatorPart lastPartForRepeater = locatorPartGenerators.lastElement()
				.lastPartForRepeaterElement(context, locatorPartGenerators.size() - 1);
