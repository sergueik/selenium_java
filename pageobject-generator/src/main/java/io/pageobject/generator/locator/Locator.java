...
public class Locator {
	...
	public static final Ordering<Locator> BY_PRIORITY_ORDERING = Ordering
			.natural().onResultOf(Locator::sortKeyFunction);

	private Locator(String locator, String locatorWhenElementIsRepeated,
			LocatorSources source) {
	...
	public static Locator fromParts(List<LocatorPart> parts,
		...
		boolean hasInvalidParts = allParts.stream()
				.anyMatch(LocatorPart::isInvalid);
		List<String> validParts = parts.stream().filter(LocatorPart::validPart)
				.map(LocatorPart::getPart).collect(toList());
		...
		return isNullOrEmpty(locator) ? null
				: new Locator(locator, locatorWhenElementIsRepeated,
						parts.get(parts.size() - 1).getSource());
	}

	private static ArrayList<LocatorPart> getAllParts(List<LocatorPart> parts,
			LocatorPart lastPartForRepeater) {
	...
	public String toString() {
		return "Locator{" + "locator='" + locator + '\''
				+ ", locatorWhenElementIsRepeated='" + locatorWhenElementIsRepeated
				+ '\'' + ", source=" + source + '}';
	}
}
