...
public enum LocatorSources {
	...
	NG_REPEAT() {
		...
		@Override
		public String extractLocatorValue(Element element,
				GeneratorContext context) {
			return allowIndexExpressionOrNoExpressions(
					extractExactRepeater(getRepeaterAttribute(element)));
		}
	...
	NG_OPTIONS() {
		...
		public String extractLocatorValue(Element element,
				GeneratorContext context) {
			return extractAttributeValueAllowingIndexExpression(element,
					HtmlAttributes.NG_OPTIONS);
		}
	...
	public String locator(String value) {
		return String.format(this.locatorTemplate(), value).replaceAll("'`", "`")
				.replaceAll("`'", "`");
	}

	public abstract String locatorTemplate();

	public abstract String extractLocatorValue(Element element,
			GeneratorContext context);
	...
	private static String extractAttributeValue(Element element,
			HtmlAttributes htmlAttribute) {
		String attributeValue = getNormalizedAttributeValue(element,
				htmlAttribute.attributeName());
		return hasExpression(attributeValue) ? null : attributeValue;
	}

	private static String extractAttributeValueAllowingIndexExpression(
			Element element, HtmlAttributes htmlAttribute) {
		String attributeValue = getNormalizedAttributeValue(element,
				htmlAttribute.attributeName());
		return allowIndexExpressionOrNoExpressions(attributeValue);
	}

	private static String allowIndexExpressionOrNoExpressions(
			String attributeValue) {
		int expressionCount = getExpressionCount(attributeValue);
		return expressionCount == 0 || (expressionCount == 1
				&& getFirstExpression(attributeValue).equals("$index")) ? attributeValue
						: null;
	}
