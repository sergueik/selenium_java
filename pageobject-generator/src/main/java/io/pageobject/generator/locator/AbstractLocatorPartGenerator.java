...
public abstract class AbstractLocatorPartGenerator
		implements LocatorPartGenerator {
	...
	public AbstractLocatorPartGenerator(Element element,
			LocatorSources... locatorCandidates) {
		checkArgument(locatorCandidates.length > 0,
				"There must be at least one locator candidate");
	...
	protected LocatorPart generateLastPart(GeneratorContext context,
			String elementFinder, int depth) {
		Element element = context.getElement();

		List<LocatorSources> sources = Arrays.stream(locatorCandidates)
				.filter(locatorSource -> locatorSource
						.isEnabledForApplicationType(context.getApplicationType()))
				.collect(toList());

		for (LocatorSources locatorCandidate : sources) {
			String attributeValue = locatorCandidate.extractLocatorValue(element,
					context);

			if (!isNullOrEmpty(attributeValue)
					&& context.isLocatorAvailable(locatorCandidate, attributeValue)) {
				StringBuilder value = new StringBuilder(elementFinder);
				value.append("(");
				value.append((locatorCandidate.locator(
						replaceIndexBindingWithFunctionParameter(attributeValue, depth))));
				value.append(")");
