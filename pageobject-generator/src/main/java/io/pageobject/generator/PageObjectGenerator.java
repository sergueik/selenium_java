...
public class PageObjectGenerator {
	...
	private final Logger logger = LoggerFactory
			.getLogger(PageObjectGenerator.class);

	static class Arguments {

		@Parameter(names = {
				"-page" }, description = "Name of the page object", required = true)
		String page;
	...
	protected Multimap<String, ElementProcessor> processors = ArrayListMultimap
			.create();
	...
	public PageObjectGenerator(ApplicationType applicationType) {
		...
		configuration.setClassForTemplateLoading(PageObjectGenerator.class,
				"templates");
	...
	public static void main(String[] args) throws Exception {
		...
		ApplicationType applicationType = arguments.type == null
				? ApplicationType.ANGULAR1 : ApplicationType.valueOf(arguments.type);
		String result = new PageObjectGenerator(applicationType)
				.runWithFileInput(arguments);
	...
	private String run(Document document, String source, String page)
			throws Exception {
		logger.info("Generating [{}] PO with page name [{}] and source [{}]",
				this.applicationType, page, source);

		GeneratorContext context = new GeneratorContext(document, source,
	...
	protected void traverse(GeneratorContext context) throws Exception {
		...
		Collection<ElementProcessor> processorsForElement = newArrayList(
				processors.get(name));
		processorsForElement.addAll(processors.get("*"));

		List<ElementProcessor> matchingProcessors = getMatchingProcessors(context,
				element, processorsForElement);
		...
		if (locator.isPresent()) {
			...
			context.markLocatorAsUsed(locatorSource,
					locatorSource.extractLocatorValue(element, context));
			...
			for (ElementProcessor elementProcessor : matchingProcessors) {
				...
				if (result != null && result.hasTemplateModel()) {
					context.addFragments(invokeTemplatesForProcessor(
							result.getTemplates(), result.getTemplateModel(), context));
		...
		if (!repeater || locator.isPresent()
				|| applicationType == ApplicationType.ANGULAR1) {
	...
	private boolean processRepeater(GeneratorContext context) throws Exception {
		...
		if (!isNullOrEmpty(repeatExpression)) {
			...
			repeatExpression = getNormalizedAttributeValue(element,
					"ng-repeat-start");
		}
	...
	private List<ElementProcessor> getMatchingProcessors(GeneratorContext context,
			Element element, Collection<ElementProcessor> processorsForElement) {
		return processorsForElement.stream()
				.filter(processor -> processor.isMatchingElement(element, context))
				.collect(toList());
	}

	private Optional<Locator> generateLocator(GeneratorContext context,
			List<ElementProcessor> matchingProcessors) {
		return matchingProcessors.stream()
				.map(processor -> processor.generateElementLocator(context))
				.filter(Objects::nonNull).min(Locator.BY_PRIORITY_ORDERING);
	}

	private Map<String, String> invokeTemplatesForProcessor(
			List<String> templates, Map<String, Object> templateModel,
			GeneratorContext context) throws IOException, TemplateException {

		Map<String, String> fragments = newLinkedHashMap();
		int controlIndex = context.getControlIndex();

		for (String template : templates) {
			String fragmentId = controlIndex + "-"
					+ template.substring(template.indexOf("/") + 1);
			String fragment = invokeTemplate(template + ".js.tpl", templateModel,
					configuration);
	...
	private String invokeTemplate(String templateName,
			Map<String, Object> templateModel, Configuration configuration)
	...
	private String beautifyJavascript(String code) {
		String beautifyJs = readScript(
				"META-INF/resources/webjars/js-beautify/1.6.2/js/lib/beautify.js");

		ScriptEngine nashorn = new ScriptEngineManager().getEngineByName("nashorn");
		try {
			CompiledScript compiledScript = ((Compilable) nashorn).compile(beautifyJs
