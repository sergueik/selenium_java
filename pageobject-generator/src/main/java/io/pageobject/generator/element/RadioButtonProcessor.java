...
public class RadioButtonProcessor extends AbstractElementProcessor {
	...
	public boolean isMatchingElement(Element element, GeneratorContext context) {
		if ("radio".equalsIgnoreCase(element.attr("type"))) {
			ArrayList<Element> elementsInGroup = findElementsInGroup(element,
					context);
	...
	private ArrayList<Element> findElementsInGroup(Element element,
		...
		for (Element radio : allRadios) {
			...
			boolean matchingNames = !isNullOrEmpty(otherName)
					&& otherName.equals(name);
			boolean matchingModels = !isNullOrEmpty(otherModel)
					&& otherModel.equals(model);
	...
	protected LocatorPartGenerator getLocatorPartGenerator(Element element,
			GeneratorContext context) {
		return new MultiElementLocatorPartGenerator(element,
				getElementLocatorCandidates(context));
	}

	public Locator generateElementLocator(GeneratorContext context) {
		ArrayList<Element> elementsInGroup = findElementsInGroup(
	...
	protected LocatorSources[] getElementLocatorCandidates(
			GeneratorContext context) {
		return new LocatorSources[] { LocatorSources.NG_MODEL, LocatorSources.NAME,
				LocatorSources.CSS };
	}
	...
	protected List<String> getTemplates(GeneratorContext context,
		...
		ArrayList<String> templates = newArrayList(CLICK_BY_INDEX, CLICK_BY_VALUE,
				ASSERTION_SELECTED_BY_INDEX, ASSERTION_SELECTED_BY_VALUE,
				ASSERTION_VISIBILITY_BY_INDEX, ASSERTION_ENABLED_BY_INDEX,
				ASSERTION_HAS_CLASS);
