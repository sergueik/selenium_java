...
public abstract class AbstractElementProcessor implements ElementProcessor {
	...
	public ProcessingResult process(GeneratorContext context) throws Exception {
		...
		boolean locatorPartGeneratorWasAdded = addLocatorPartGeneratorToContext(
				context, element);
		...
		templateModel.put("referenceToElement",
				context.isNestedElement() ? locator : "this." + name);
		templateModel.put("functionArguments", generateFunctionArguments(context));
		templateModel.put("functionArgumentsCount",
				context.getLocatorPartGenerators().size() - 1);
	...
	protected boolean addLocatorPartGeneratorToContext(GeneratorContext context,
			Element element) {
		LocatorPartGenerator locatorPartGenerator = getLocatorPartGenerator(element,
				context);
	...
	protected LocatorPartGenerator getLocatorPartGenerator(Element element,
		...
		return new SingleElementLocatorPartGenerator(element,
				getElementLocatorCandidates(context));
	}

	protected LocatorSources[] getElementLocatorCandidates(
			GeneratorContext context) {
		return new LocatorSources[] { LocatorSources.ID, LocatorSources.NAME,
				LocatorSources.NG_MODEL, LocatorSources.CSS };
	}

	public Locator generateElementLocator(GeneratorContext context) {
		boolean generatorWasAdded = addLocatorPartGeneratorToContext(context,
				context.getElement());
	...
	protected String getLocator(GeneratorContext context) {
		...
		return isNullOrEmpty(locator.getLocatorWhenElementIsRepeated())
				? locator.getLocator() : locator.getLocatorWhenElementIsRepeated();
	}

	private String generateFunctionArguments(GeneratorContext context) {
		Stack<LocatorPartGenerator> locatorPartGenerators = context
				.getLocatorPartGenerators();
	...
	protected void populateTemplateModel(Map<String, Object> templateModel,
			GeneratorContext context) {

	}

	protected abstract List<String> getTemplates(GeneratorContext context,
	...
	protected String getLastElementOfTranslationKey(String text) {
		...
		return key.contains(".")
				? key.substring(key.lastIndexOf('.') + 1, key.length() - 1) : key;
	}

	protected ProcessingResult getNormalResult(Map<String, Object> templateModel,
			String... templates) {
	...
	protected ProcessingResult getTerminatedResult(
			Map<String, Object> templateModel, String... templates) {
