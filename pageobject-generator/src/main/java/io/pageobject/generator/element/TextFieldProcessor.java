...
public class TextFieldProcessor extends AbstractElementProcessor {
	...
	public boolean isMatchingElement(Element element, GeneratorContext context) {
		String type = element.attr("type") == null ? null
				: element.attr("type").toLowerCase();
		return "text".equals(type) || "tel".equals(type) || "search".equals(type)
				|| "url".equals(type) || "email".equals(type) || "password".equals(type)
				|| "number".equals(type) || "range".equals(type);
	}
