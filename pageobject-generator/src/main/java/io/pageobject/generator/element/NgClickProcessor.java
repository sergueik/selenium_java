...
public class NgClickProcessor extends AbstractElementProcessor {
	...
	public boolean isMatchingElement(Element element, GeneratorContext context) {
		...
		return !isNullOrEmpty(ngClick) && !isLink(element) && !isButton(element)
				&& !isInputButton(element);
	}
	...
	protected LocatorSources[] getElementLocatorCandidates(
		...
		ArrayList<LocatorSources> sources = newArrayList(LocatorSources.ID,
				LocatorSources.NAME, LocatorSources.NG_MODEL, LocatorSources.NG_OPTIONS,
				LocatorSources.CSS);
