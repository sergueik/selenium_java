...
public class Elements {
	...
	public static String generateCssSelector(Element element,
		...
		int numberOfDirectChildren = getNumberOfChildren(directChildrenSelector,
				parent);

		if (context.isNestedElement()) {
			int numberOfAllChildren = getNumberOfChildren(selector.toString(),
					parent);
		...
		if (numberOfDirectChildren > 1) {
			org.jsoup.select.Elements childrenWithSameSelector = parent
					.select(directChildrenSelector);
			String type = childrenWithSameSelector.get(0).tagName();
			org.jsoup.select.Elements childrenWithSameType = parent
					.select(">" + type);
			int siblingIndex = childrenWithSameType.indexOf(element);

			if (anyPrecedingSiblingIsAStopElement(childrenWithSameType,
					siblingIndex)) {
				return null;
			}
	...
	private static boolean anyPrecedingSiblingIsAStopElement(
			org.jsoup.select.Elements childrenWithSameType, int siblingIndex) {
	...
	private static boolean hasReachedLastRepeaterElement(GeneratorContext context,
			Element parent) {
		return context.isNestedElement()
				&& context.getLastRepeaterElement() == parent;
	}
