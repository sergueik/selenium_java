...
public class Expressions {
	...
	public static String extractNameFromRepeater(String repeaterExpression) {
		String[] variableAndExpression = extractExactRepeater(repeaterExpression)
				.split(" in ");
		return variableAndExpression.length == 1 ? ""
				: variableAndExpression[1].trim();
	}

	public static String extractExactRepeater(String repeaterExpression) {
		return repeaterExpression.split(" track by ")[0].split(" as ")[0]
				.split("\\|")[0].split("=")[0].trim();
	}

	public static String extractNameFromNgFor(String ngForExpression) {
		String[] ofSplit = ngForExpression.split(";")[0].split("\\|")[0]
				.split(" of ");
		return ofSplit.length == 1 ? "" : ofSplit[1].trim();
	}

	public static String replaceIndexBindingWithFunctionParameter(String text,
			int repeaterCount) {
		if (text.contains("{{$index}}")) {
			return "`" + text.replaceAll("\\{\\{\\$index\\}\\}",
					"\\${rowIndex" + repeaterCount + "}") + "`";
		} else {
			return text;
		}
	}
