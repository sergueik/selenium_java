...
public class NameExtractorTest {
	...
	public void convertsToCamelCase() throws Exception {
		assertThat(getName("id", "name-with-multiple-parts"))
				.isEqualTo("nameWithMultipleParts");
		assertThat(getName("id", "name1.with.multiple.parts"))
				.isEqualTo("name1WithMultipleParts");
		assertThat(getName("id", "name2:with:multiple:parts"))
				.isEqualTo("name2WithMultipleParts");
		assertThat(getName("id", "name3_with_multiple_parts"))
				.isEqualTo("name3WithMultipleParts");
		assertThat(getName("id", "name4_with-multiple:parts"))
				.isEqualTo("name4WithMultipleParts");
	}

	@Test
	public void complexName() throws Exception {
		assertThat(getName("id", "'name_with'{{b}}-multiple{{i}}:parts[0] | piped"))
				.isEqualTo("nameWithMultipleParts");
	}

	@Test
	public void removesAccents() throws Exception {
		assertThat(getName("id", "árvíztűrőtükörfúrógép"))
				.isEqualTo("arvizturotukorfurogep");
	}

	@Test
	public void respectsAttributeOrder() throws Exception {
		assertThat(getName("id", "ignored", "name", "name", "ng-model", "ignored"))
				.isEqualTo("name");
		assertThat(getName("id", "ignored", "name", "{{i}}", "ng-model", "name1"))
				.isEqualTo("name1");
		assertThat(getName("id", "name2", "name", "[0]", "ng-model", "{{i}}"))
				.isEqualTo("name2");
	}
	...
	public void handlesPostfixes() throws Exception {
		...
		assertThat(getName(Tag.valueOf("a"), "id", "nameLink"))
				.isEqualTo("nameLink");
		assertThat(getName(Tag.valueOf("a"), "id", "nameButton"))
				.isEqualTo("nameButton");
		assertThat(getName(Tag.valueOf("button"), "id", "name1"))
				.isEqualTo("name1Button");
		assertThat(getName(Tag.valueOf("button"), "id", "name1Button"))
				.isEqualTo("name1Button");
	}
	...
	public void handlesKeyValueRepeaters() throws Exception {
		assertThat(getName("ng-repeat", "(name, age) in {'adam':10, 'amalie':12}"))
				.isEqualTo("adam10Amalie12");
	}
	...
	public void removesControllerPrefixes() throws Exception {
		...
		assertThat(nameExtractor.extractValue(element, context))
				.isEqualTo("myControllerItem2");

		element.attr("title", "todoListName2");
		assertThat(nameExtractor.extractValue(element, context))
				.isEqualTo("todoListName2");
	}
