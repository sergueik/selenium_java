...
public class ElementLocatorGeneratorTest {
	...
	public void simpleElements() throws Exception {
		assertThat(getSingleElementLocator("id", "name"))
				.isEqualTo("element(by.id('name'))");
		assertThat(getSingleElementLocator("name", "name"))
				.isEqualTo("element(by.name('name'))");
		assertThat(getSingleElementLocator("ng:model", "name"))
				.isEqualTo("element(by.model('name'))");
		assertThat(getSingleElementLocator("ng_model", "name1"))
				.isEqualTo("element(by.model('name1'))");
		assertThat(getSingleElementLocator("ng_options", "name1"))
				.isEqualTo("element(by.options('name1'))");
	}

	@Test
	public void multipleElements() throws Exception {
		assertThat(getMultiElementLocator("id", "name"))
				.isEqualTo("element.all(by.id('name'))");
		assertThat(getMultiElementLocator("name", "name"))
				.isEqualTo("element.all(by.name('name'))");
		assertThat(getMultiElementLocator("ng:model", "name"))
				.isEqualTo("element.all(by.model('name'))");
		assertThat(getMultiElementLocator("ng_model", "name1"))
				.isEqualTo("element.all(by.model('name1'))");
	}

	@Test
	public void respectsAttributeOrder() throws Exception {
		assertThat(getSingleElementLocator("id", "name", "name", "ignored"))
				.isEqualTo("element(by.id('name'))");
		assertThat(getSingleElementLocator("id", "name1", "name", "ignored",
				"ng_model", "ignored")).isEqualTo("element(by.id('name1'))");
		assertThat(getSingleElementLocator("name", "name", "ng-model", "ignored"))
				.isEqualTo("element(by.name('name'))");
	}
	...
	public void sameLocatorIsOnlyUsedOnce() throws Exception {
		...
		assertThat(getSingleElementLocator("id", "name", "name", "fallback"))
				.isEqualTo("element(by.name('fallback'))");
	}
	...
	public void handlesElementsInRepeaters() throws Exception {
		...
		context.pushNgRepeat(repeaterElement, "item in items");
		assertThat(getSingleElementLocator("id", "name"))
				.isEqualTo("this.items.get(rowIndex1).element(by.id('name'))");
		context.pushNgRepeat(createElement(), "element in elements");
		assertThat(getSingleElementLocator("id", "name", "name", "name1"))
				.isEqualTo(
	...
	public void handlesNgRepeatElement() throws Exception {
		...
		context.pushNgRepeat(repeaterElement, "item in items");
		assertThat(getLocator(repeaterElement))
				.isEqualTo("element.all(by.exactRepeater('item in items'))");
	}
	...
	public void handlesSimpleElementThatHasRepeaterAsWell() throws Exception {
		...
		context.setElement(repeaterElement);
		context.pushNgRepeat(repeaterElement, "item in items");
		assertThat(getLocatorWhenElementIsRepeated(repeaterElement, "id", "name"))
				.isEqualTo("this.items.get(rowIndex1)");
	}
	...
	public void handlesNgRepeatWithMultipleElements() throws Exception {
		...
		context.pushNgRepeat(repeaterElement, "item in items");
		assertThat(getMultiElementLocator("id", "name"))
				.isEqualTo("this.items.get(rowIndex1).all(by.id('name'))");
		context.pushNgRepeat(createElement(), "element in elements");
	...
	public void handlesIndexExpressions() throws Exception {
		...
		assertThat(getSingleElementLocator("ng-model", "model{{$index}}"))
				.isEqualTo(
						"this.items.get(rowIndex1).element(by.model(`model${rowIndex1}`))");
		assertThat(getSingleElementLocator("ng-options",
				"color.name for color in colors{{$index}}")).isEqualTo(
	...
	private String getSingleElementLocator(Element element,
			String... attributes) {
		Locator locator = getLocator(new SingleElementLocatorPartGenerator(element,
				LocatorSources.ID, LocatorSources.NAME, LocatorSources.NG_MODEL,
				LocatorSources.NG_OPTIONS), element, attributes);
	...
	private String getMultiElementLocator(String... attributes) {
		...
		return getLocator(new MultiElementLocatorPartGenerator(element,
				LocatorSources.ID, LocatorSources.NAME, LocatorSources.NG_MODEL,
				LocatorSources.NG_OPTIONS), element, attributes).getLocator();
	}
	...
	private String getLocatorWhenElementIsRepeated(Element element,
			String... attributes) {
		return getLocator(null, element, attributes)
				.getLocatorWhenElementIsRepeated();
	}

	private Locator getLocator(LocatorPartGenerator locatorPartGenerator,
			Element element, String... attributes) {
