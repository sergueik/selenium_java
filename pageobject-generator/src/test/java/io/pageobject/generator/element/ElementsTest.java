...
public class ElementsTest {
	...
	public void cssSelectorWithId() throws Exception {
		GeneratorContext context = getContext(
				"<div><div id=\"id1\">A <div>B <div>C</div></div></div></div>");
		Document document = context.getDocument();
		...
		assertThat(generateCssSelector(divC, context))
				.isEqualTo("#id1 > div > div");
	}

	@Test
	public void cssSelectorWithClasses() throws Exception {
		GeneratorContext context = getContext(
				"<div><div class=\"a b\">A <div class=\"c\">B <div>C</div></div></div></div>");
		Document document = context.getDocument();
		...
		assertThat(generateCssSelector(divB, context))
				.isEqualTo("div > div.a.b > div.c");
		assertThat(generateCssSelector(divC, context))
				.isEqualTo("div > div.a.b > div.c > div");
	}

	@Test
	public void cssSelectorWithNamespacedElements() throws Exception {
		GeneratorContext context = getContext(
				"\"<html><body><fb:comments /></body></html>\"");
		Document document = context.getDocument();

		org.jsoup.select.Elements comments = document.select("fb|comments");

		assertThat(generateCssSelector(comments.get(0), context))
				.isEqualTo("body > fb|comments");
	}
	...
	public void cssSelectorWithMultipleChildren() throws Exception {
		...
		assertThat(generateCssSelector(divA, context))
				.isEqualTo("div > div:nth-of-type(1)");
		assertThat(generateCssSelector(divB, context))
				.isEqualTo("div > div:nth-of-type(2)");
		assertThat(generateCssSelector(divC, context))
				.isEqualTo("div > div.a:nth-of-type(3)");
		assertThat(generateCssSelector(divD, context))
				.isEqualTo("div > div.a:nth-of-type(4)");
	}

	@Test
	public void cssSelectorStopsAtRepeater() throws Exception {
		GeneratorContext context = getContext(
				"<div>A <div ng-repeat=\"item in items\">B <div class=\"a\">C <div>D</div></div></div></div>");
		Document document = context.getDocument();
		...
		context.pushLocatorPartGenerator(
				new SingleElementLocatorPartGenerator(divC, LocatorSources.ID));
	...
	@Test
	public void cssSelectorDoesNotGenerateSelectorIfNestedRepeaterHasSimilarElements()
			throws Exception {
		GeneratorContext context = getContext(
	...
	@Test
	public void cssSelectorDoesNotGenerateAnythingIfPrecedingSiblingHasStopAttribute()
			throws Exception {
	...
	private void tryStopElement(String attribute) throws Exception {
		GeneratorContext context = getContext("<div><div>A</div><div " + attribute
				+ "=\"a\">B</div><div>C</div></div>");
		Document document = context.getDocument();
