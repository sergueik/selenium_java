...
public class PageObjectGeneratorTest {
	...
	private void runTest(final String page, final String expectedPageObject)
			throws Exception {
	...
	private void runTest(final String page, final String expectedPageObject,
		...
		String path = Paths
				.get(this.getClass().getResource("/" + page + ".html").toURI())
				.toString();

		PageObjectGenerator.Arguments arguments = new PageObjectGenerator.Arguments();
		new JCommander(arguments, "-page", StringUtils.capitalize(page), "-file",
				path);

		String pageObject = new PageObjectGenerator(applicationType)
				.runWithFileInput(arguments);

		File expectedFile = new File(this.getClass()
				.getResource("/" + expectedPageObject + ".js").getPath());
