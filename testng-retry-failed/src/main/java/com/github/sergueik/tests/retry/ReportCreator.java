...
public class ReportCreator {
	...
	public static void addTestReport(String className, String testName,
			String status) {
		if (status == "Failure") {
	...
	public static void addCommonRunMetrics(int totalCount, int successCount,
			int failureCount) {
	...
	public static void addFailedTestsBlock(String className, String testName,
			String status) {
	...
	public static void addfailedWithStacktraces(String className, String testName,
			String status, String stackTrace) {
		Div failedWithStackTraces = new Div().setCSSClass("Lowest");
		failedWithStackTraces
				.appendText(className + " " + testName + " " + status + "\n");
	...
	public static String getReport() {
		...
		Link cssStyle = new Link().setType("text/css").setRel("stylesheet")
				.setHref("src/main/resources/site.css");
		...
		for (int s = 0; s < list.size(); s++) {
			if (list.get(s).getTestResult() == "Failure") {
				addFailedTestsBlock(list.get(s).getClassName(),
						list.get(s).getTestName(), list.get(s).getTestResult());
			}
		}
		if (list.isEmpty()) {
			...
			if (!isClassConstructed) {
				for (int k = 0; k < list.size(); k++) {
					if (currentTestClass.equals(list.get(k).getClassName())) {
						addTestReport(list.get(k).getClassName(), list.get(k).getTestName(),
								list.get(k).getTestResult());
					}
				}
		...
		for (int z = 0; z < list.size(); z++) {
			if (list.get(z).getTestResult() == "Failure") {
				addfailedWithStacktraces(list.get(z).getClassName(),
						list.get(z).getTestName(), list.get(z).getTestResult(),
						list.get(z).getStackTrace());
			}
		}
