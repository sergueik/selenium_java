...
public class NaginatorListener extends RunListener<AbstractBuild<?, ?>> {
	...
	public void onCompleted(AbstractBuild<?, ?> build,
		...
		// Do nothing for null or a single Matrix run. (Run only when all Matrix
		// finishes)
		...
		List<NaginatorScheduleAction> actions = build
				.getActions(NaginatorScheduleAction.class);
			...
			if (action.shouldSchedule(build, listener, retryCount)) {
					...
					if (build instanceof MatrixBuild) {
						...
						for (MatrixRun r : matrixRuns) {
							if (r.getNumber() == build.getNumber()) {
								...
								LOGGER.log(Level.FINE, "add combination to matrix rerun ({0})",
										r.getParent().getCombination().toString());
				...
				if (!combsToRerun.isEmpty()) {
					...
					case RerunEmpty:
						...
						break;
					case DontRun:
						continue; // confusing, but back to the look for
	...
	public Environment setUpEnvironment(
		...
		return new Environment() {
			...
			public void buildEnvVars(Map<String, String> env) {
				...
				env.put("NAGINATOR_MAXCOUNT",
						Integer.toString(action.getMaxRetryCount()));
	...
	public boolean scheduleMatrixBuild(AbstractBuild<?, ?> build,
		...
		return scheduleMatrixBuild(build, combinations, n,
				NaginatorListener.calculateRetryCount(build), 0);
	}

	private boolean scheduleMatrixBuild(AbstractBuild<?, ?> build,
			List<Combination> combinations, int delay, int retryCount,
			int maxRetryCount) {
		NaginatorMatrixAction nma = new NaginatorMatrixAction(build, retryCount,
				maxRetryCount);
	...
	private boolean scheduleBuild(AbstractBuild<?, ?> build, int n,
