...
public class NaginatorListenerTest extends HudsonTestCase {
	...
	public void testSuccessNoRebuild() throws Exception {
		assertEquals(false,
				isScheduledForRetry("build log", Result.SUCCESS, "foo", false, false));
	}
	...
	public void testUnstableWithRebuild() throws Exception {
		assertEquals(true,
				isScheduledForRetry("build log", Result.UNSTABLE, "foo", true, false));
	}

	public void testFailureWithRebuild() throws Exception {
		assertEquals(true,
				isScheduledForRetry("build log", Result.FAILURE, "foo", false, false));
	}

	public void testFailureWithUnstableRebuild() throws Exception {
		assertEquals(true,
				isScheduledForRetry("build log", Result.FAILURE, "foo", true, false));
	}

	public void testFailureWithoutRebuildRegexp() throws Exception {
		assertEquals(false,
				isScheduledForRetry("build log", Result.FAILURE, "foo", false, true));
	}

	public void testFailureWithRebuildRegexp() throws Exception {
		assertEquals(true, isScheduledForRetry("build log foo", Result.FAILURE,
				"foo", false, true));
	}

	public void testUnstableWithoutRebuildRegexp() throws Exception {
		assertEquals(false,
				isScheduledForRetry("build log", Result.UNSTABLE, "foo", true, true));
	}

	public void testUnstableWithRebuildRegexp() throws Exception {
		assertEquals(true, isScheduledForRetry("build log foo", Result.UNSTABLE,
				"foo", true, true));
	}
	...
	public void testWithBuildWrapper() throws Exception {
		...
		NaginatorPublisher nag = new NaginatorPublisher("foo", false, false, false,
				false, 10, new FixedDelay(0));
	...
	public void testRetainCauses() throws Exception {
		...
		NaginatorPublisher nag = new NaginatorPublisher("", false, false, false,
				false, 2, new FixedDelay(1));
	...
	private boolean isScheduledForRetry(String buildLog, Result result,
			String regexpForRerun, boolean rerunIfUnstable, boolean checkRegexp)
			throws Exception {
		@SuppressWarnings("deprecation")
		FreeStyleProject project = createFreeStyleProject();
		project.getBuildersList().add(new MyBuilder(buildLog, result));
		Publisher nag = new NaginatorPublisher(regexpForRerun, rerunIfUnstable,
				false, checkRegexp, false, 10, new FixedDelay(0));
		project.getPublishersList().add(nag);

		return isScheduledForRetry(project);
	}

	private boolean isScheduledForRetry(FreeStyleProject project)
			throws Exception {
	...
	public void testCountScheduleIndependently() throws Exception {
		...
		p.getPublishersList()
				.add(new NaginatorPublisher("", // regexpForRerun
		...
		p.scheduleBuild2(0, new Cause.UserIdCause(),
				new ParametersAction(new StringParameterValue("PARAM", "A")));
		p.scheduleBuild2(0, new Cause.UserIdCause(),
				new ParametersAction(new StringParameterValue("PARAM", "B")));

		waitUntilNoActivity();

		assertEquals(3,
				Collections2.filter(p.getBuilds(), new Predicate<FreeStyleBuild>() {
					public boolean apply(FreeStyleBuild b) {
						try {
							return "A"
									.equals(b.getEnvironment(TaskListener.NULL).get("PARAM"));
						} catch (IOException e) {
							return false;
	...
	public void testCatastorophicRegularExpression() throws Exception {
		...
		p.getBuildersList()
				.add(new MyBuilder(
						...
						Result.FAILURE));
		p.getPublishersList()
				.add(new NaginatorPublisher("(0*)*NOSUCHSTRING", // regexpForRerun
						false, // rerunIfUnstable
						false, // rerunMatrixPart
						true, // checkRegexp
						false, // maxScheduleOverrideAllowed
		...
		((NaginatorPublisher.DescriptorImpl) jenkins
				.getDescriptor(NaginatorPublisher.class)).setRegexpTimeoutMs(1000);
	...
	public static class VariableRecordBuilder extends Builder {
		...
		public boolean perform(AbstractBuild<?, ?> build, Launcher launcher,
			...
			recorded.put(getIdForBuild(build),
					build.getEnvironment(listener).get(name));
			return true;
		}
	}
	...
	public void testVariable() throws Exception {
		FreeStyleProject p = createFreeStyleProject();
		VariableRecordBuilder countRecorder = new VariableRecordBuilder(
				"NAGINATOR_COUNT");
		VariableRecordBuilder maxCountRecorder = new VariableRecordBuilder(
				"NAGINATOR_MAXCOUNT");
		VariableRecordBuilder buildNumberRecorder = new VariableRecordBuilder(
		...
		assertEquals("1",
				buildNumberRecorder.getRecordedValue(p.getBuildByNumber(2)));
		...
		assertEquals("2",
				buildNumberRecorder.getRecordedValue(p.getBuildByNumber(3)));
	}

	public void testVariableForMatrixBuild() throws Exception {
		@SuppressWarnings("deprecation")
		MatrixProject p = jenkins.createProject(MatrixProject.class,
		...
		// for the first build (not a retrying build)
		assertNull(countRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value1"))));
		assertNull(maxCountRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value1"))));
		assertNull(buildNumberRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value1"))));
		assertNull(countRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value2"))));
		assertNull(maxCountRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value2"))));
		assertNull(buildNumberRecorder.getRecordedValue(p.getBuildByNumber(1)
				.getExactRun(new Combination(axisList, "value2"))));
	...
	public void testMavenModuleSetWithoutNaginator() throws Exception {
		final String SIMPLE_POM = StringUtils
				.join(new String[] { "<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
						"<project xmlns=\"http://maven.apache.org/POM/4.0.0\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://maven.apache.org/POM/4.0.0 http://maven.apache.org/maven-v4_0_0.xsd\">",
						"  <modelVersion>4.0.0</modelVersion>",
						"  <groupId>com.exmaple</groupId>",
						"  <artifactId>test</artifactId>", "  <version>1.0</version>",
		...
		p.addProperty(
				new PrepareSingleFileProperty("pom.xml", SIMPLE_POM.getBytes("UTF-8")));
		p.setGoals("clean");
