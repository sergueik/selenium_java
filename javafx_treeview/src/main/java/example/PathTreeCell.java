	...
	public PathTreeCell(final Stage owner, final StringProperty messageProp) {
		...
		expandAllMenu.setOnAction(new EventHandler<ActionEvent>() {
			...
			private void expandTreeItem(TreeItem<PathItem> item) {
				...
				children.stream().filter(child -> (!child.isLeaf())).forEach(child -> expandTreeItem(child));
			}
		});
		...
		addMenu.setOnAction(new EventHandler<ActionEvent>() {
			...
			private Path createNewDirectory() {
				...
				while (true) {
					...
					newDir = Paths.get(path.toAbsolutePath().toString(),
		...
		deleteMenu.setOnAction((ActionEvent event) -> {
			...
			prop.addListener((ObservableValue<? extends TreeItem<PathItem>> ov, TreeItem<PathItem> oldItem,
				...
				try {
					...
					messageProp
							.setValue(String.format("Deleting %s failed", newItem.getValue().getPath().getFileName()));
				}
			});
		});
