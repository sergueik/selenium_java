...
/*
 Exception in thread "main-AWTAnimator#00" com.jogamp.opengl.util.AnimatorBase$ 
 UncaughtAnimatorException: com.jogamp.opengl.GLException: 
 Caught GLException: No shader code found (source nor binary) for src: [shaders/gl3/hello-triangle.vert],
 bin: null on thread main-AWTAnimator#00 

 https://learnopengl.com/Getting-started/Shaders
 http://antongerdelan.net/opengl/hellotriangle.html
 https://github.com/sgothel/jogl/blob/master/src/jogl/classes/com/jogamp/opengl/util/glsl/ShaderCode.java#L551
 */
public class TriangleDemoEx implements GLEventListener, KeyListener {

	private static Animator animator;
	private static GLWindow topWindow;
	private static final String profile = GLProfile.GL2GL3;

	private float[] vertexData = { -1, -1, 1, 0, 0, +0, +2, 0, 0, 1, +1, -1, 0, 1,
			0 };
	...
	private void setup() {
		...
		topWindow = GLWindow.create(glCapabilities);

		topWindow.setTitle("Hello Triangle (simple)");
		topWindow.setSize(1024, 768);

		topWindow.setVisible(true);

		// instead of anonymous
		topWindow.addGLEventListener(this);
		topWindow.addKeyListener(this);

		topWindow.addWindowListener(new WindowAdapter() {
			@Override
			public void windowDestroyed(WindowEvent e) {
				animator.stop();
				// need a handle to TriangleDemoEx
				// e.getSource().
				// this.dispose(drawable);
				System.exit(1);
			}
		});
		animator = new Animator(topWindow);
		animator.start();
	}
	...
	private void initProgram(GL3 gl) {
		...
		program = new Program(gl, getClass(), "shaders/gl3", "hello-triangle",
				"hello-triangle");
	...
	private class Program {
		...
		Program(GL3 gl, Class context, String root, String vertex,
				String fragment) {

			ShaderCode vertShader = ShaderCode.create(gl, GL_VERTEX_SHADER,
					this.getClass(), root, null, vertex, "vert", null, true);
			ShaderCode fragShader = ShaderCode.create(gl, GL_FRAGMENT_SHADER,
					this.getClass(), root, null, fragment, "frag", null, true);
			...
			gl.glUniformBlockBinding(name, globalMatricesBI,
					Semantic.Uniform.GLOBAL_MATRICES);
		}
	}
	...
	private void initBuffers(GL3 gl) {
		...
		gl.glBindBufferBase(GL_UNIFORM_BUFFER, Semantic.Uniform.GLOBAL_MATRICES,
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl.glClearBufferfv(GL_COLOR, 0,
				clearColor.put(0, 0f).put(1, .33f).put(2, 0.66f).put(3, 1f));
		...
		// model matrix
		{
			...
			float[] scale = FloatUtil.makeScale(new float[16], true, 0.5f, 0.5f,
					0.5f);
			float[] zRotation = FloatUtil.makeRotationEuler(new float[16], 0, 0, 0,
					diff);
