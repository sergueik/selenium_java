...
public class Config {
	...
	ConfigKey<String> properties = new ConfigKey<>("wdm.properties", String.class,
	...
	ConfigKey<String> gitHubTokenName = new ConfigKey<>("wdm.gitHubTokenName",
			String.class);
	ConfigKey<String> gitHubTokenSecret = new ConfigKey<>("wdm.gitHubTokenSecret",
			String.class);
	...
	public boolean isExecutable(File file) {
		return resolve(os).equalsIgnoreCase("win")
				? file.getName().toLowerCase().endsWith(".exe") : file.canExecute();
	}
	...
	public String getDriverVersion(String name) {
		return resolver(name, driverVersion.getValue(), driverVersion.getType());
	}
