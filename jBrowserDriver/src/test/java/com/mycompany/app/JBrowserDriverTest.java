...
public class JBrowserDriverTest {
	...
	public void test1SendKeys() {
		...
		assertThat(element.getAttribute("class"), containsString("progress-bar-success"));
	...
	public void test2SendKeys() {
		...
		element = driver.findElement(By.cssSelector("div#container form#file_form input#file"));
		assertThat(element, notNullValue());
		highlight(element);
		...
		element = driver.findElement(By.cssSelector("div#container form#file_form input#submit"));
		element.click();
	}
	...
	public void testExecutePhantomJS() {
		if (driver instanceof PhantomJSDriver) {
			...
			((PhantomJSDriver) driver).executePhantomJS(
					String.format("var page = this; page.uploadFile('input[id=file]', '%s' );", testFilePath));
			try {
				Thread.sleep(1000);
			} catch (InterruptedException e) {
	...
	public void verifyTextTest() throws Exception {
		try {
			...
			verificationErrors.append("Error in verifyTextTest() : " + e.toString() + "\n");
		}
	}
	...
	public void test13_1() {
		...
		wait.until(ExpectedConditions
				.frameToBeAvailableAndSwitchToIt(By.cssSelector(".container .row .intro-message iframe")));

		// Act
		WebElement buttonElement = wait
				.until(ExpectedConditions.visibilityOf(driver.findElement(By.cssSelector("h3 button"))));
	...
	public void cssSelectorOfElementTest() throws Exception {
		try {
			...
			verificationErrors.append("Error in cssSelectorOfElementTest() : " + e.toString() + "\n");
		}
	}
	...
	public void cssSelectorOfElementWithIdTest() throws Exception {
		try {
			...
			verificationErrors.append("Error in cssSelectorOfElementWithIdTest() : " + e.toString() + "\n");
		}
	}
	...
	public void testcssSelectorOfElementAlternative() throws Exception {
		...
		try {
			...
			assertEquals("form[name=\"PTPT_HAC_FORM\"] > div > label > input[name=\"q\"]", selector);
		} catch (Error e) {
			verificationErrors.append("Error in testcssSelectorOfElementAlternative() : " + e.toString() + "\n");
		}
		try {
			element = findElement("css_selector", selector);
			highlight(element);
	...
	private List<WebElement> findElements(String selectorKind, String selectorValue, WebElement parent) {
		...
		if (selectorKind == null || !supportedSelectorStrategies.containsKey(selectorKind)
	...
	private List<WebElement> findElements(String selectorKind, String selectorValue) {
	...
	protected static String getScriptContent(String scriptName) throws Exception {
		try {
			final InputStream stream = JBrowserDriverTest.class.getClassLoader().getResourceAsStream(scriptName);
