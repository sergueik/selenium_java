...
public class NgLocalFileTest {
	...
	public void testDatePickerDirectSelect() throws Exception {
		...
		List<WebElement> ng_weekDates = ng_dropdown.findElements(NgBy
				.repeater("dateObject in week.dates"));
		...
		dateElement.click();
		NgWebElement ng_element = ng_dropdown.findElement(NgBy
				.model("data.inputOnTimeSet"));
		assertThat(ng_element, notNullValue());
		List<WebElement> ng_dataDates = ng_element.findElements(NgBy
	...
	public void testDatePickerNavigation() throws Exception {
		...
		assertThat(ng_dropdown, notNullValue());
		highlight(ng_dropdown);
		NgWebElement ng_display = ngDriver.findElement(NgBy
				.binding("data.previousViewDate.display"));
		...
		ArrayList<String> months = new ArrayList<>(Arrays.asList(new String[] {
		...
		WebElement ng_right = ng_display.findElement(By.xpath("..")).findElement(
				By.className("right"));
		assertThat(ng_right, notNullValue());
		highlight(ng_right);
		ng_right.click();
		ng_display = ngDriver.findElement(NgBy
				.binding("data.previousViewDate.display"));
		highlight(ng_display);
	...
	public void testUpload1() throws Exception {
		...
		WebElement file = ngDriver.findElement(By
				.cssSelector("div[ng-controller = 'myCtrl'] > input[type='file']"));
	...
	public void testUpload3() throws Exception {
		...
		// assertThat(result, notNullValue());
		wait.until(ExpectedConditions.visibilityOf(ngDriver.findElement(By
				.cssSelector("input[ type='button' ][ value='Upload' ]"))));
	...
	public void testEvaluateEvenOdd() throws Exception {
		...
		Enumeration<WebElement> rows = Collections.enumeration(ngDriver
				.findElements(NgBy.repeater("x in names")));

		while (rows.hasMoreElements()) {
			WebElement currentRow = rows.nextElement();

			Enumeration<WebElement> cells = Collections.enumeration(currentRow
	...
	public void testFindRepeaterElement() throws Exception {
		...
		Matcher matcher = pattern.matcher(ngDriver.findElement(
				NgBy.binding("filtered.length")).getText());
		if (matcher.find()) {
			length = Integer.parseInt(matcher.group(1).toString());
		}
		for (int cnt = 0; cnt != length; cnt++) {
			Enumeration<WebElement> cells = Collections.enumeration(ngDriver
					.findElements(NgBy.repeaterElement("client in filtered ", cnt,
							"client.name")));
			while (cells.hasMoreElements()) {
				WebElement currentCell = cells.nextElement();
		...
		List<WebElement> elements = ngDriver.findElements(NgBy.repeaterElement(
				"client in filtered ", length, "client.name"));
	...
	public void testFindSelectedtOptionWithAlert() throws Exception {
		...
		Iterator<WebElement> options = ngDriver.findElements(
		...
		WebElement countSelected = ngDriver.findElement(NgBy
				.binding("countSelected"));
		assertThat(countSelected, notNullValue());
		// System.err.println(countSelected.getText() );
		int valueOfCountSelected = Integer.parseInt(new NgWebElement(ngDriver,
				countSelected).evaluate("countSelected").toString());
	...
	public void testChangeSelectedtOption() throws Exception {
		...
		getPageContent("ng_select_array.htm");
		Iterator<WebElement> options = ngDriver.findElements(
				NgBy.repeater("option in options")).iterator();
		while (options.hasNext()) {
			WebElement option = (WebElement) options.next();
			System.err.println("available option: " + option.getText());
			if (option.getText().isEmpty()) {
	...
	public void testChangeSelectedRepeaterOption() throws Exception {
		...
		Iterator<WebElement> options = ngDriver.findElements(
				NgBy.repeater("fruit in Fruits")).iterator();
		while (options.hasNext()) {
			WebElement option = (WebElement) options.next();
			System.err.println("available option: " + option.getText());
			if (option.getText().isEmpty()) {
		...
		List<WebElement> elements = ngDriver.findElements(NgBy
				.selectedRepeaterOption("fruit in Fruits"));
		try {
			assertThat(elements.size(), equalTo(1));
		} catch (AssertionError e) {
			if (isCIBuild) {
		...
		List<NgWebElement> ngElements = ngRootWevelement.findNgElements(NgBy
				.selectedRepeaterOption("fruit in Fruits"));
		try {
			assertThat(elements.size(), equalTo(1));
		} catch (AssertionError e) {
		...
		ngElement = ngDriver.findElement(NgBy
				.selectedRepeaterOption("fruit in Fruits"));
		assertThat(ngElement, notNullValue());
		assertThat(ngElement.getWrappedElement(), notNullValue());
		System.err.println("selected option: " + ngElement.getText());
	...
	public void testMultiSelect2() throws Exception {
		...
		WebElement button = ngDriver
				.findElement(By
						.cssSelector("multiselect-dropdown button[data-ng-click='openDropdown()']"));
		...
		elements = ngDriver.findElements(NgBy.repeaterColumn(
				"country in SelectedCountries", "country.name"));
	...
	public void testMultiSelect() throws Exception {
		...
		String selectedValues = ngDriver
				.findElement(NgBy.binding("selectedValues")).getText();
	...
	public void testFindElementByRepeaterWithBeginEnd() throws Exception {
		...
		List<WebElement> elements = ngDriver.findElements(NgBy
				.repeater("definition in definitions"));
	...
	public void testFindElementByOptions() throws Exception {
		...
		List<WebElement> elements = ngDriver.findElements(NgBy
				.options("c.name for c in colors"));
	...
	public void testFindElementByModel() throws Exception {
		...
		WebElement pattern = ngDriver.findElement(NgBy
				.binding("form.value.$error.pattern"));
		assertThat(pattern.getText(), containsString("false"));
		System.err.println(pattern.getText()); // pattern: false
		WebElement required = ngDriver.findElement(NgBy
				.binding("!!form.value.$error.required"));
		...
		assertThat(pattern.getText(), containsString("false"));
		System.err.println(pattern.getText()); // pattern: false
		required = ngDriver.findElement(NgBy
				.binding("!!form.value.$error.required"));
	...
	public void testDropDownWatch() throws Exception {
		...
		List<WebElement> elementsStates = ngDriver.findElements(NgBy
		...
		List<WebElement> cities = ngDriver.findElements(NgBy
				.options("city for city in cities"));
	...
	public void testFindRepeaterRows() throws Exception {
		...
		List<WebElement> todos = ngDriver.findElements(NgBy.repeaterRows(
				todos_repeater, 1));
		...
		List<WebElement> todos_check = ngDriver.findElements(NgBy
				.repeater(todos_repeater));
		todos = ngDriver.findElements(NgBy.repeaterRows(todos_repeater,
				todos_check.size() + 1));
		assertThat(todos.size(), equalTo(0));
	}
	...
	public void testOrderByColumnField() throws Exception {
		...
		for (String header : headers) {
			for (int cnt = 0; cnt != 2; cnt++) {
				...
				WebElement headerelement = ngDriver.findElement(By
						.xpath("//th/a[contains(text(),'" + header + "')]"));
				headerelement.click();
				// Then Angular uses the column <binding> to sort rows in <sort order>
				// order
				List<WebElement> rows = ngDriver.findElements(NgBy
						.repeater("emp in data.employees"));
				...
				boolean reverseSort = parseBoolean(ng_row.evaluate("reverseSort")
						.toString());
				// Without Protractor:
				...
				while (iteratorEmp.hasNext()) {
					...
					try {
						WebElement empFieldElement = ng_emp.findElement(NgBy
								.binding(binding));
				...
				WebElement first_row_cell = ngDriver.findElement(NgBy.repeaterElement(
						"emp in data.employees", 0, binding));
	...
	public void testAngularUISelectHandleSelectedAndAvailable() throws Exception {
		...
		search.click();
		ngDriver.waitForAngular();
		List<WebElement> availableColors = ngDriver.findElements(By
		...
		while (iteratorAvailableColors.hasNext()) {
			...
			try {
				availableColorIndex = Integer.parseInt(ngAvailableColor.evaluate(
						"$index").toString());
			} catch (Exception e) {
			}
	...
	public void testAngularUISelectHandleSearch() throws Exception {
		...
		while (iteratorAvailableColors.hasNext()) {
			...
			System.err.println("search = " + searchText + ", found :"
					+ availableColor.getText());
		}
	}
	...
	public void testAngularUISelectHandleDeselect() throws Exception {
		...
		while (selectedColors.size() != 0) {
			selectedColors = ngDriver.findElements(NgBy
					.repeater("$item in $select.selected"));
			Iterator<WebElement> iteratorSelectedColors = selectedColors.iterator();
			while (iteratorSelectedColors.hasNext()) {
				WebElement selectedColor = (WebElement) iteratorSelectedColors.next();
				...
				WebElement ng_close = ngSelectedColor.findElement(By
						.cssSelector("span[class *='close']"));
	...
	public void testFindAllBindings() throws Exception {
		...
		WebElement container = ngDriver.getWrappedDriver().findElement(
	...
	public void testHover() throws InterruptedException {
		...
		try {
			wait.until(ExpectedConditions.visibilityOf(seleniumDriver.findElement(By
					.cssSelector("div[ng-show='hovering']"))));
		} catch (NoSuchElementException e) {
		}
	}
	...
	public void testDropDown() throws Exception {
		...
		while (iteratorCountries.hasNext()) {
			...
			assertTrue(country.getAttribute("value").matches(
					"(?i:India|Australia|Usa)"));
		...
		while (iteratorStates.hasNext()) {
			...
			assertTrue(state.getAttribute("value").matches(
					"(?i:New South Wales|Victoria)"));
	...
	public void testDragAndDrop() throws InterruptedException, Exception {
		...
		WebElement basket = seleniumDriver.findElement(By
				.xpath("//*[@id='my-basket']"));
		...
		while (ng_cars.hasMoreElements()) {
			...
			List<WebElement> ng_cars_basket = ng_basket.findElements(NgBy
							.repeater("car in models.basket"));
