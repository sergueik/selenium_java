...
import example.utils.CellUtil;
import example.utils.SheetUtil;
...
public class AutoExcel {
	...
	public static void save(String templatePath, String outputPath, List<TemplateExportPara> templateExportParas,
			Consumer<Workbook> actionAhead, Consumer<Workbook> actionBehind) throws Exception {
		...
		List<BlockNameResolver<TemplateExportPara>> blockNameResolvers = resolveCellNames(workbook,
				templateExportParas);
		...
		for (TemplateExportPara templateExportPara : templateExportParas) {
			if (templateExportPara.getDataSource() != null && templateExportPara.getRecordCount() > 0)
				dataSourceNameFields.put(templateExportPara.getDataSourceName().toLowerCase(),
						mapFieldNameField(templateExportPara.getObjectType()));
		}
		...
		for (BlockNameResolver<TemplateExportPara> blockNameResolver : blockNameResolvers) {
			...
			if (templateExportPara.getDataSourceType() == DataSourceType.List) {
				if (templateExportPara.isInserted() && recordCount > 1) {
					int startRow = blockNameResolver.getFieldNameCells().entrySet().iterator().next().getValue()
							.getRowIndex() + 1;
					// Move the other rows to free up enough space to populate the current data
					// source
					sheet.shiftRows(startRow, sheet.getLastRowNum(), recordCount - 1, true, false);
					// After the row is moved, the original cell position changes and the cell
					// location saved in the cell
				...
				if (templateExportPara.getDataDirection() == DataDirection.Right) {
					int startColIndex = blockNameResolver.getFieldNameCells().entrySet().iterator().next().getValue()
							.getColIndex();
	...
	public static void save(String outputPath, List<DirectExportPara> directExportParas) throws Exception {
		...
		for (DirectExportPara directExportPara : directExportParas) {
			...
			if (fieldSettings == null || fieldSettings.size() == 0) {
				fieldSettings = fieldNameFields.values().stream().map(m -> new FieldSetting(m.getName(), m.getName()))
			...
			for (FieldSetting fieldSetting : fieldSettings) {
				SheetUtil.setValue(sheet, rowIndex, colIndex, fieldSetting.getDisplayName()).setCellStyle(headStyle);
				++colIndex;
			}
			// write data
			++rowIndex;
	...
	private static <T extends TemplatePara> List<BlockNameResolver<T>> resolveCellNames(Workbook workbook,
			List<T> paras) {
		...
		// Iterate over all cell names, categorize by data source name, and resolve the
		// column and row indexes corresponding to cell names
		for (Name name : names) {
			...
			/*
			 * format of cell name： 1. dataSourceName.fieldName[.aggregateType], eg.
			 * cb_product.SaleArea.sum 2. dataSourceName.formula.xxxx, eg.
			 * cb_product.formula.1 3. dataSourceName.RowNo, eg. cb_product.RowNo
			 */
			...
			if (!currDataSourceName.equalsIgnoreCase(arr[0])) {
				...
				Optional<T> para = paras.stream()
						.filter(m -> m.getDataSourceName().equalsIgnoreCase(finalCurrDataSourceName)).findFirst();
			...
			if (arr.length == 3) {
				if (secondPart.equals("formula")) {
					if (CellType.forInt(cell.getCellType()) == CellType.FORMULA) {
	...
	private static void reLocate(Workbook workbook, List<BlockNameResolver<TemplateExportPara>> blockNameResolvers,
	...
	private static void reLocate(Workbook workbook, Pattern pattern,
			Collection<? extends BaseCellManager> baseCellManagers) {
	...
	private static void writeRecordByCellName(Sheet sheet, BlockNameResolver<TemplateExportPara> templateExcelManager,
			Object dataSourceRecord, Map<String, Map<String, Field>> dataSourceNameFields, int step)
		...
		for (Map.Entry<String, CellManager> entry : templateExcelManager.getFieldNameCells().entrySet()) {
			...
			if (dataSourceNameFields.containsKey(dataSourceName)
					&& dataSourceNameFields.get(dataSourceName).containsKey(fieldName)) {
	...
	private static void writeRecordByFieldSetting(Sheet sheet, List<FieldSetting> fieldSettings,
			Object dataSourceRecord, Map<String, Field> fieldNameFields, int rowIndex) throws IllegalAccessException {
		int colIndex = 0;
		for (FieldSetting fieldSetting : fieldSettings) {
	...
	private static void writeAggregate(Sheet sheet, BlockNameResolver<TemplateExportPara> templateExcelManager,
			int recordCount) {
		for (String fieldName : templateExcelManager.getFieldNameAggregateCells().keySet()) {
			AggregateCellManager aggregateCellManager = templateExcelManager.getFieldNameAggregateCells()
					.get(fieldName);
			Cell aggregateCell = SheetUtil.getOrCreateCell(sheet, aggregateCellManager.getRowIndex(),
					aggregateCellManager.getColIndex());
			if (templateExcelManager.getFieldNameCells().containsKey(fieldName)) {
				...
				case SUM:
					if (templateExcelManager.getPara().getDataDirection() == DataDirection.Down)
						aggregateCell.setCellFormula(
								String.format("SUM(%1$s%2$s:%1$s%3$s)", colName, rowIndex + 1, rowIndex + recordCount));
					else
						aggregateCell.setCellFormula(String.format("SUM(%1$s%2$s:%3$s%2$s)", colName, rowIndex + 1,
								indexToColName(colIndex + recordCount - 1)));
					break;
				case AVG:
					if (templateExcelManager.getPara().getDataDirection() == DataDirection.Down)
						aggregateCell.setCellFormula(String.format("AVERAGE(%1$s%2$s:%1$s%3$s)", colName, rowIndex + 1,
								rowIndex + recordCount));
					else
						aggregateCell.setCellFormula(String.format("AVERAGE(%1$s%2$s:%3$s%2$s)", colName, rowIndex + 1,
								indexToColName(colIndex + recordCount - 1)));
					break;
				case NONE:
					break;
	...
	private static void writeFormula(Sheet sheet, BlockNameResolver<TemplateExportPara> templateExcelManager,
			int recordCount) {
	...
	public static Map<String, List<Map<String, Object>>> read(String fileName, List<ImportPara> importParas)
			throws Exception {
	...
	public static Map<String, List<Map<String, Object>>> read(FileInputStream fileInputStream,
			List<ImportPara> importParas) throws Exception {
		Workbook workbook = new XSSFWorkbook(fileInputStream);
		return read(workbook, importParas);
	}

	private static Map<String, List<Map<String, Object>>> read(Workbook workbook, List<ImportPara> importParas)
			throws Exception {
		...
		for (BlockNameResolver<ImportPara> blockNameResolver : blockNameResolvers) {
			if (blockNameResolver.getPara() == null)
				continue;
			...
			case Down:
				for (int i = rowNum; i <= sheet.getLastRowNum(); ++i) {
					...
					if (row == null)
						continue; // default null if the row doesn't contain data
